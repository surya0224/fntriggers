...
class AdvancedPurchase(RuleChange):
    """
...
def main():
    client = mongo_client()
    new_df = get_new_rules_df(cat='005', record_3='JUP_DB_ATPCO_Record_3_Cat_05',
                              fields={'TKTG_PERIOD': 1, 'TKTG_UNIT': 1}, client=client)
    if new_df is not None:
        old_df = get_old_rules_df(new_df=new_df, record_3='JUP_DB_ATPCO_Record_3_Cat_05',
                                  fields={'TKTG_PERIOD': 1, 'TKTG_UNIT': 1}, client=client)
        ...
        df = get_host_rules(df=df, cat='cat_5', client=client)
        ...
        for idx, row in df[df['ACTION_new'] == '3'].iterrows():
            for rule in row['host_rules']:
                old_doc = {'fare_rule': rule, 'tariff_code': row['TARIFF'], 'cat_no': row['CAT_NO'],
                           'tktg_period': row['TKTG_PERIOD_old'], 'tktg_unit': row['TKTG_UNIT_old'], 'comp_fare_rule': row['RULE_NO'],
                           'comp_seq': row['SEQ_NO'], 'eff_from': row['DATES_EFF_1_old'],
                           'eff_to': row['DATES_DISC_1_old'], 'competitor': row['CXR_CODE'],
                           'record_3': 'JUP_DB_ATPCO_Record_3_Cat_05'}
                new_doc = {'fare_rule': rule, 'tariff_code': row['TARIFF'], 'cat_no': row['CAT_NO'],
                           'tktg_period': row['TKTG_PERIOD_new'], 'tktg_unit': row['TKTG_UNIT_new'], 'comp_fare_rule': row['RULE_NO'],
                           'comp_seq': row['SEQ_NO'], 'eff_from': row['DATES_EFF_1_new'],
                           'eff_to': row['DATES_DISC_1_new'], 'competitor': row['CXR_CODE'],
                           'record_3': 'JUP_DB_ATPCO_Record_3_Cat_05'}
                obj = AdvancedPurchase(name='competitor_advanced_purchase_change',
