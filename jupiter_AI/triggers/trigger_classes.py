...
from jupiter_AI.batch.fbmapping_batch.fbmapping_batch_old import get_host_fareladder
...
from jupiter_AI.network_level_params import SYSTEM_DATE, Host_Airline_Code
...
from jupiter_AI.triggers.recommendation_models.oligopoly import get_price_recommendation
...
class trigger(object):
    ...
    def __init__(self, triggering_event_id):
        ...
        self.tiles = {
            "market_impact_ageing(days)" : 10,
            "volatility_factor(percentage)" : 20,
            "market_penetration_index" : 3,
            "external_evaluation_matrix" : "On Target",
            "price_stability_index" : 0,
            "strategy" : "Differentiation",
            "collaboration_index" : "",
            "price_war" : "Yes",
            "internal_evaluation_matrix" : "Good",
            "market_elasticity" : 1
        }
        error_flag = False
        print error_flag
    ...
    def check_min_interval(self):
        ...
        '''
        q = db.JUP_DB_Cum_Pricing_Actions.find({'pos': self.pos,
                                                'origin': self.origin,
                                                'destination': self.destination,
    ...
    def update_db(self):
        ...
        pricing_actions = update_host_workflow_params_comp(trigger_obj=pricing_actions)
        pricing_actions = update_host_workflow_params_fb(trigger_obj=pricing_actions)
        pricing_actions = update_market_workflow_params(trigger_obj=pricing_actions)
        print 'DONE'
        pricing_actions['status'] = None
        pricing_actions['action_date'] = None
        pricing_actions['action_time'] = None
        print pricing_actions
        db.JUP_DB_Workflow_1_dummy.insert(self.__dict__)
...
class competitor_rating_change(thesis_model_competitor):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
        thesis_model_competitor.__init__(self, triggering_event_id)
        self.get_desc()
...
class competitor_market_share_change(thesis_model_competitor):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
        thesis_model_competitor.__init__(self, triggering_event_id)
        self.get_desc()
...
# class competitor_exit(thesis_model_competitor):
# @measure(JUPITER_LOGGER)
# def __init__(self,triggering_event_id):
# 		thesis_model_competitor.__init__(self,triggering_event_id)
# 		self.get_desc()
# @measure(JUPITER_LOGGER)
# def get_desc(self):
# 		self.desc = ' '.join(("Recommendation for trigger raised because a competitor",
# 							self.old_doc_data['airline'].encode(),"has exited the market on",
# 							str(self.triggering_event_date)," at ",str(self.triggering_event_time)))


# class competitor_new_entry(thesis_model_competitor):
# @measure(JUPITER_LOGGER)
# def __init__(self,triggering_event_id):
# 		thesis_model_competitor.__init__(self,triggering_event_id)
# 		self.get_desc()
# @measure(JUPITER_LOGGER)
# def get_desc(self):
# 		self.desc = ' '.join(("Recommendation for trigger raised because a new competitor",
# 							self.old_doc_data['airline'].encode(),"has arised in the following OD ","origin-",
# 							self.old_doc_data['origin']," destination-",self.old_doc_data['destination']," on",
# 							str(self.triggering_event_date)," at ",str(self.triggering_event_time)))
...
class host_rating_change(thesis_model_host):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
        thesis_model_host.__init__(self, triggering_event_id)
        self.get_desc()
...
class competitor_airline_capacity_percentage_change(thesis_model_competitor):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
        thesis_model_competitor.__init__(self, triggering_event_id)
        self.get_desc()
...
class host_airline_capacity_percentage_change(thesis_model_host):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
        thesis_model_host.__init__(self, triggering_event_id)
        self.get_desc()
...
class forecast_changes(thesis_model_host):
    """
    """
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
...
class booking_changes_weekly(data_level):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
...
class booking_changes_VLYR(data_level):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
        data_level.__init__(self, triggering_event_id)
    ...
    def get_desc(self):
        desc = [
            ...
            '_',
            'Bkgs Last Year', self.old_doc_data['bookings'],
            '_',
            'Capacity Last Year', self.old_doc_data['capacity'],
            '_',
            'Capacity This Year', self.new_doc_data['capacity'],
            '_',
            'Expected Bkgs This Year', self.new_doc_data['bookings_expected'],
            '_',
            'Actual Bkgs This Year', self.new_doc_data['bookings'],
            '_',
            'Thresholds:' + str(self.lower_threshold) + ' to ' + str(self.upper_threshold),
            '_',
            'Raised On ' + str(self.triggering_event_date) + ' at ' + str(self.triggering_event_time)
...
class booking_changes_VTGT(data_level):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
        data_level.__init__(self, triggering_event_id)
    ...
    def get_desc(self):
        ...
        self.desc = ''.join(desc)
        # self.desc = ' '.join(["Recommendation for trigger raised for the departure period[",
        #                      str(self.triggering_data['dep_date_start']), " to ", str(self.triggering_data['dep_date_end']),
        #                      "] wherein the variance of percentage of target achieved with bookings this year(",
        #                      str(round(self.new_doc_data['percent_acheived']*100,2)), '%)'
        #                      " with respect to that of the last year (",
        #                      str(round(self.old_doc_data['percent_acheived']*100,2)), '%) '
        #                      "is beyond thresholds(", str(self.lower_threshold), "::",
...
class yield_changes_VTGT(data_level):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
        data_level.__init__(self, triggering_event_id)
    ...
    def get_desc(self):
        self.desc = ' '.join((
                             "Recommendation for trigger raised because the % change of Yield this week with respect to the target is beyond thresholds as checked on ",
                             str(self.triggering_event_date), " at ", str(self.triggering_event_time)))
...
class lowest_fare_comparision(data_level):
    @measure(JUPITER_LOGGER)
    def __init__(self, triggering_event_id):
        data_level.__init__(self, triggering_event_id)
