...
from jupiter_AI import parameters, MONGO_CLIENT_URL, MONGO_SOURCE_DB, ANALYTICS_MONGO_PASSWORD, \
    ANALYTICS_MONGO_USERNAME, RABBITMQ_USERNAME, RABBITMQ_PASSWORD, RABBITMQ_HOST, RABBITMQ_PORT, client, mongo_client, JUPITER_LOGGER
...
if __name__ == "__main__":
    ...
    with rabbitpy.Connection(url) as conn:
        with conn.channel() as channel:
            queue = rabbitpy.Queue(channel, 'Queue9')
            ...
            try:
                ...
                for message in queue:
                    ...
                    try:
                        message.ack()
                        processing_function(message.json(), db=db)
#                    	message.ack()
                    except SyntaxError:
                        print "PASS"
                        pass
