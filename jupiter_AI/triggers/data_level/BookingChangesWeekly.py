...
Long Description for Trigger:
    Let Observation Date - '2017-02-15'
        Parameters in Comparision -
            1.  Bookings obtained for departure dates b/w '2017-02-01' to '2017-02-28'(this month) in this week
                '2017-02-08' to '2017-02-15'(observation date)
            2.  Bookings obtained for departure dates b/w '2017-01-23' to '2017-02-23' till
...
@measure(JUPITER_LOGGER)
def generate_booking_changes_weekly_triggers(pos,
                                             origin,
                                             destination,
                                             compartment,
                                             data_list,
    ...
    try:
        ...
        if sig_flag:
            ...
            trigger_obj1 = BookingChangesWeekly(data, SYSTEM_DATE)
            trigger_obj1.do_analysis(dep_date_start=SYSTEM_DATE,
                                     dep_date_end=dep_date_end1,
                                     db=db,
                                     data_list=data_list)
            print "time for 1st object: ", time.time() - st
            st = time.time()
            ...
            trigger_obj2 = BookingChangesWeekly(data, SYSTEM_DATE)
            trigger_obj2.do_analysis(dep_date_start=dep_date_start2,
                                     dep_date_end=dep_date_end2,
                                     db=db,
                                     data_list=data_list)
            print "time for 2nd object: ", time.time() - st
            st = time.time()
            ...
            trigger_obj3 = BookingChangesWeekly(data, SYSTEM_DATE)
            trigger_obj3.do_analysis(dep_date_start=dep_date_start3,
                                     dep_date_end=dep_date_end3,
                                     db=db,
                                     data_list=data_list)
            print "time for 3rd object: ", time.time() - st
            st = time.time()
            ...
            trigger_obj4 = BookingChangesWeekly(data, SYSTEM_DATE)
            trigger_obj4.do_analysis(dep_date_start=dep_date_start4,
                                     dep_date_end=dep_date_end4,
                                     db=db,
                                     data_list=data_list)
            print "time for 4th object: ", time.time() - st
    except Exception as error_msg:
        # print traceback.print_exc()
        module_name = ''.join(
            [
                'jupiter_AI/triggers/data_level/BookingChangesWeekly.py ',
                'method: generate_booking_changes_weekly_triggers'])
    ...
    return BKWK_TRIGGERS


class BookingChangesWeekly(DataLevel):
    """
    ...
    def __init__(self, data, system_date):
        super(BookingChangesWeekly, self).__init__(data, system_date)
    ...
    def do_analysis(self, dep_date_start, dep_date_end, data_list, db, is_event_trigger=False):
        ...
        # print booking_start_date_tw
        # print booking_end_date_tw
        # print dep_date_start
        # print dep_date_end
        ...
        self.get_trigger_details(trigger_name='booking_changes_weekly', db=db)
        ...
        for tup1, tup2 in zip(bookings_data_tw.sort_values(by='market').iterrows(),
            ...
            try:
                ...
                BKWK_TRIGGERS.append(id)
            except Exception as e:
                db.JUP_DB_Errors.insert({"err_id": "BKWK",
    ...
    def check_trigger(self, bookings_data_tw, bookings_data_lw):
        ...
        if (self.old_doc_data['bookings']) != 0 and self.old_doc_data['bookings'] != "NA" and self.new_doc_data[
            ...
            if self.threshold_type == "percent":
                # old_doc_nz['change'] = (old_doc_nz['bookings_new_doc'] - old_doc_nz['bookings_old_doc'])*100/old_doc_nz['bookings_old_doc']
                self.change = (
...
def main_helper(db):
    ...
    for mrkt in list(sig_markets_df['market'].values):
        if counter == 2000:
            id_list = generate_booking_changes_weekly_triggers(data_list=markets,
                                                               pos=data['pos'],
                                                               origin=data['origin'],
