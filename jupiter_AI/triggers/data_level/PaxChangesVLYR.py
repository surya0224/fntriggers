"""
File Name              :   PaxChangesVLYR.py
Author                 :   Sai Krishna
Date Created           :   2016-02-21
Description            :   Generation of a PaxChangesVLYR trigger for a market
...
Long Description for Trigger:
    Let Observation Date - '2017-02-15'
        Parameters in Comparision -
            ...
            2.  Pax obtained for departure dates b/w '2017-02-01' to '2017-02-28' till
...
@measure(JUPITER_LOGGER)
def generate_pax_changes_vlyr_triggers(
        pos,
        origin,
        destination,
        compartment,
    ...
    try:
        ...
        if sig_flag:
            ...
            if len(dates_list) > 0:
                for date in dates_list:
                    ...
                    trigger_obj.do_analysis(book_date_start=book_date_start,
            ...
            trigger_obj1 = PaxChangesVLYR(data, SYSTEM_DATE)
            trigger_obj1.do_analysis(book_date_start=book_date_start,
                                     book_date_end=book_date_end,
            ...
            trigger_obj2 = PaxChangesVLYR(data, SYSTEM_DATE)
            trigger_obj2.do_analysis(book_date_start=book_date_start,
                                     book_date_end=book_date_end,
                                     dep_date_start=dep_date_start2,
            ...
            trigger_obj3 = PaxChangesVLYR(data, SYSTEM_DATE)
            trigger_obj3.do_analysis(book_date_start=book_date_start,
                                     book_date_end=book_date_end,
                                     dep_date_start=dep_date_start3,
            ...
            trigger_obj4 = PaxChangesVLYR(data, SYSTEM_DATE)
            trigger_obj4.do_analysis(book_date_start=book_date_start,
                                     book_date_end=book_date_end,
                                     dep_date_start=dep_date_start4,
        ...
        obj_error = error_class.ErrorObject(
            ...
            'jupter_AI/triggers/data_level/PaxChangesVLYR.py method: generate_pax_changes_vlyr_triggers',
            get_arg_lists(
                inspect.currentframe()))
        obj_error.append_to_error_list(str(error_msg))
        obj_error.write_error_logs(datetime.datetime.now())
    return PXVL_TRIGGERS


class PaxChangesVLYR(DataLevel):
    @measure(JUPITER_LOGGER)
    def __init__(self, data, system_date):
        super(PaxChangesVLYR, self).__init__(data, system_date)
    ...
    def do_analysis(
        ...
        print pax_data
        self.get_trigger_details(trigger_name='pax_changes_VLYR', db=db)
        for tup1 in pax_data.iterrows():
            try:
                ...
                PXVL_TRIGGERS.append(id)
            except Exception as e:
                db.JUP_DB_Errors.insert({"err_id": "PXVL",
    ...
    @measure(JUPITER_LOGGER)
    def check_trigger(self, pax_ly, pax_ty):
        ...
        if self.old_doc_data['pax'] != 0 and pax_ty != "NA" and pax_ly != "NA":
            ...
            #     # self.new_doc_data['pax_expected'] = self.new_doc_data['pax']
            # else:
            #     self.new_doc_data['pax_expected'] = self.old_doc_data['pax']
            ...
            if self.change < self.lower_threshold or self.change > self.upper_threshold:
                ...
                return False
        elif self.old_doc_data['pax'] == 0 and \
             self.new_doc_data['pax'] != 0:
            self.change = 'inf'
            # self.new_doc_data['pax_expected'] = self.new_doc_data['pax']
            return True
        else:
...
def main_helper(db):
    ...
    for mrkt in list(sig_markets_df['market'].values):
        if counter == 2000:
            id_list = generate_pax_changes_vlyr_triggers(data_list=markets,
                                                                pos=data['pos'],
                                                                origin=data['origin'],
