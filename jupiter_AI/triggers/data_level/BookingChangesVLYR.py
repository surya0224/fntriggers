"""
File Name              :   BookingChangesVLYR.py
Author                 :   Sai Krishna
Date Created           :   2016-02-21
Description            :   Generation of a BookingChangesVLYR trigger for a market
...
Long Description for Trigger:
    Let Observation Date - '2017-02-15'
        Parameters in Comparision -
            1.  Bookings obtained for departure dates b/w '2017-02-01' to '2017-02-28'(this month) till
                '2017-02-15'(observation date)
            2.  Bookings obtained for departure dates b/w '2017-02-01' to '2017-02-28' till
                this observation date last year
            1.  Capacity This YR '2017-02-01' to '2017-02-28'
            2.  Capacity Last YR for the '2016-02-01' to '2016-02-28'
...
@measure(JUPITER_LOGGER)
def generate_booking_changes_vlyr_triggers(pos,
                                           origin,
                                           destination,
                                           compartment,
                                           data_list,
    ...
    try:
        ...
        if sig_flag:
            ...
            trigger_obj1 = BookingChangesVLYR(data, SYSTEM_DATE)
            ...
            trigger_obj2 = BookingChangesVLYR(data, SYSTEM_DATE)
            ...
            trigger_obj3 = BookingChangesVLYR(data, SYSTEM_DATE)
            ...
            trigger_obj4 = BookingChangesVLYR(data, SYSTEM_DATE)
        ...
        obj_error = error_class.ErrorObject(
            ...
            'jupter_AI/triggers/data_level/BookingChangesVLYR.py method: generate_booking_changes_vlyr_triggers',
            get_arg_lists(
                inspect.currentframe()))
        obj_error.append_to_error_list(str(error_msg))
        obj_error.write_error_logs(datetime.datetime.now())
    return BKVL_TRIGGERS


class BookingChangesVLYR(DataLevel):
    @measure(JUPITER_LOGGER)
    def __init__(self, data, system_date):
        super(BookingChangesVLYR, self).__init__(data, system_date)
    ...
    def do_analysis(self, dep_date_start, dep_date_end, data_list, db, is_event_trigger=False):
        ...
        book_date_end_obj = datetime.datetime.strptime(
            self.trigger_date, '%Y-%m-%d')
        book_date_end = self.trigger_date
        book_date_start = str(book_date_end_obj.year - 100) + '-01-01'

        #   Getting Bookings data till Obs Date
        # st = time.time()
        bookings_data = self.get_bookings_data(book_date_start=book_date_start,
                                               ...
                                               db=db,
                                               data_list=data_list)
        # print "time to get bookings data: ", time.time()-st
        # print 'bookings'
        # print bookings_data

        #   Getting Capacity Data between the two departure dates
        # st = time.time()
        # capacity_data = self.get_capacity_data(dep_date_start=dep_date_start,
        #                                        dep_date_end=dep_date_end,
        #                                        data_list=data_list)
        # print "time to get capacity data: ", time.time() - st
        # print 'capacity'
        # print capacity_data

        #   Getting trigger details for booking changes vlyr
        self.get_trigger_details(trigger_name='booking_changes_VLYR', db=db)
        ...
        # st = time.time()
        for idx, tup1 in bookings_data.iterrows():
            # print tup2
            try:
                self.old_doc_data['origin'] = tup1['market'][3:6]
                self.old_doc_data['destination'] = tup1['market'][6:9]
                self.old_doc_data['pos'] = tup1['market'][0:3]
                self.old_doc_data['compartment'] = tup1['market'][9:]
                self.new_doc_data = deepcopy(self.old_doc_data)
                trigger_occurence = self.check_trigger(
                    booking_ly=tup1['bookings_ly'],
                    booking_ty=tup1['bookings_ty'],
                    bookings_ty_exp=tup1['bookings_ty_exp'],
                    capacity_ly=tup1['capacity_ly'],
                    capacity_ty=tup1['capacity_ty'])
                ...
                BKVL_TRIGGERS.append(id)
            except Exception as e:
                db.JUP_DB_Errors.insert({"err_id": "BKVL",
    ...
    @measure(JUPITER_LOGGER)
    def check_trigger(self, booking_ly, booking_ty, capacity_ly, capacity_ty, bookings_ty_exp):
        """
        To Check the parameters, check with the thresholds and returns a bool whether a trigger is fired or not
        :param booking_ly:
        :param booking_ty:
        :param capacity_ly:
        :param capacity_ty:
        ...
        # print ("old: ", self.old_doc_data['bookings'])
        # print ("new: ", self.new_doc_data['bookings'])
        # print ("cap_old: ", capacity_ly)
        # print ("cap_new: ", capacity_ty)
        if self.old_doc_data['bookings'] != 0 and booking_ly != "NA" and booking_ty != "NA":
            if self.old_doc_data['capacity'] and self.new_doc_data['capacity'] and self.new_doc_data[
                ...
                self.new_doc_data['bookings_expected'] = round(
                    float(bookings_ty_exp))
            else:
                self.new_doc_data['bookings_expected'] = self.old_doc_data['bookings']
            if self.threshold_type == "percent":
                self.change = (
                                      self.new_doc_data['bookings_expected'] - self.old_doc_data['bookings']) * 100 / \
                              self.old_doc_data['bookings']
            else:
                self.change = (
                        self.new_doc_data['bookings_expected'] -
                        self.old_doc_data['bookings'])
            ...
            if self.change < self.lower_threshold or self.change > self.upper_threshold:
                ...
                return False
        elif self.old_doc_data['bookings'] == 0 and \
                self.new_doc_data['bookings'] != 0 and \
                self.old_doc_data['capacity'] and \
                self.new_doc_data['capacity'] and \
                self.new_doc_data['capacity'] != 'NA' and \
                self.old_doc_data['capacity'] != 'NA':

            self.change = "inf"
            self.new_doc_data['bookings_expected'] = math.ceil(
                float(
                    self.new_doc_data['bookings']) *
                self.old_doc_data['capacity'] /
                self.new_doc_data['capacity'])
            return True
        else:
            # print("Error occured!!")
            module_name = ''.join(['jupiter_AI/triggers/data_level/BookingChangesVLYR.py ',
            ...
            no_lst_yr_bookings_error_desc = ''.join(
                ['Bookings Value for last year for the given market', str(self.old_doc_data)])
...
def main_helper(db):
    ...
    for mrkt in ['UAETBSDXBY']:
        if counter == 2000:
            id_list = generate_booking_changes_vlyr_triggers(data_list=markets,
                                                             pos=data['pos'],
                                                             origin=data['origin'],
