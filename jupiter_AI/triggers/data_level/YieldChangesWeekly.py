"""
File Name              :   YieldChangesWeekly.py
Author                 :   Sai Krishna
...
@measure(JUPITER_LOGGER)
def generate_yield_changes_weekly_triggers(pos,
                                           origin,
                                           destination,
                                           compartment,
                                           db,
    ...
    try:
        ...
        if sig_flag:
            ...
            trigger_obj1 = YieldChangesWeekly(data, SYSTEM_DATE)
            ...
            trigger_obj2 = YieldChangesWeekly(data, SYSTEM_DATE)
            ...
            trigger_obj3 = YieldChangesWeekly(data, SYSTEM_DATE)
            ...
            trigger_obj4 = YieldChangesWeekly(data, SYSTEM_DATE)
        ...
        obj_error = error_class.ErrorObject(
            ...
            'jupiter_AI/triggers/data_level/YieldChangesWeekly.py method: generate_yield_changes_weekly_triggers',
            get_arg_lists(
...
class YieldChangesWeekly(DataLevel):
    """
    ...
    def __init__(self, data, system_date):
        super(YieldChangesWeekly, self).__init__(data, system_date)
    ...
    def do_analysis(self, dep_date_start, dep_date_end, db, data_list, is_event_trigger=False):
        ...
        self.get_trigger_details(trigger_name='yield_changes_weekly', db=db)
        for tup1, tup2 in zip(yield_data_tw.sort_values(by='market').iterrows(),
            ...
            #   Check whether the trigger is fired or not
                ...
                YWK_TRIGGERS.append(id)
            except Exception as e:
                db.JUP_DB_Errors.insert({"err_id": "YIWK",
...
def main_helper(db):
    ...
    for mrkt in list(sig_markets_df['market'].values):
        if counter == 2000:
            id_list = generate_yield_changes_weekly_triggers(data_list=markets,
                                                                pos=data['pos'],
                                                                origin=data['origin'],
