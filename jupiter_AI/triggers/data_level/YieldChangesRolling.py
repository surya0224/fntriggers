"""
File Name              :   YieldChangesRolling.py
Author                 :   Sai Krishna
Date Created           :   2016-02-07
Description            :   Generation of a YieldChangesRolling trigger for a market
...
Long Description for Trigger:
    Let Observation Date - '2017-02-15'
        Parameters in Comparision -
            1.  Yield obtained for departure dates b/w '2017-02-01' to '2017-02-28'(this month) till
                '2017-02-15'(observation date)
            2.  Yield obtained for departure dates b/w '2017-01-23' to '2017-02-23' till
                '2017-02-15'.
...
@measure(JUPITER_LOGGER)
def generate_yield_changes_rolling_triggers(pos,
                                            origin,
                                            destination,
                                            compartment,
                                            data_list,
    ...
    try:
        ...
        if sig_flag:
            ...
            if len(dates_list) > 0:
                for date in dates_list:
                    trigger_obj = YieldChangesRolling(data, SYSTEM_DATE)
            ...
            trigger_obj1 = YieldChangesRolling(data, SYSTEM_DATE)
            ...
            trigger_obj2 = YieldChangesRolling(data, SYSTEM_DATE)
            ...
            trigger_obj3 = YieldChangesRolling(data, SYSTEM_DATE)
            ...
            trigger_obj4 = YieldChangesRolling(data, SYSTEM_DATE)
        ...
        obj_error = error_class.ErrorObject(
            ...
            'jupiter_AI/triggers/data_level/YieldChangesRolling.py method: generate_yield_changes_rolling_triggers',
            get_arg_lists(
...
class YieldChangesRolling(DataLevel):
    """
    Class object that defines Yield Changes Rolling Trigger
    Arguments:
    ...
    def __init__(self, data, system_date):
        super(YieldChangesRolling, self).__init__(data, system_date)
    ...
    def do_analysis(self, dep_date_start, dep_date_end, data_list, db, is_event_trigger=False):
        ...
        """
        #   Obtaining yield period consideration till Obs date
        ...
        #   Getting Yield data till today
        yield_data_tw = self.get_yield_data(book_date_start=book_start_date_tw,
                                            book_date_end=book_end_date_tw,
                                            dep_date_start=dep_date_start,
                                            dep_date_end=dep_date_end,
                                            db=db,
                                            data_list=data_list)
        # print 'yield_tw', yield_data_tw

        #   Triggering data represents departure date ranges and yield date ranges under consideration
        ...
        #   yield start date lw = year start date
        #   yield end date lw = yield end date this week(obs date) - 7 days
        ...
        #   Obtaining yield data for last weeks parameters
        yield_data_lw = self.get_yield_data(book_date_start=book_start_date_lw,
                                            book_date_end=book_end_date_lw,
                                            dep_date_start=dep_date_start_lw,
                                            dep_date_end=dep_date_end_lw,
                                            db=db,
                                            data_list=data_list)
        # print 'yield_lw', yield_data_lw

        #   Generating triggering data for this last weeks params
        self.triggering_data['book_date_start_lw'] = book_start_date_lw
        self.triggering_data['book_date_end_lw'] = book_end_date_lw
        self.triggering_data['dep_date_start_lw'] = dep_date_start_lw
        self.triggering_data['dep_date_end_lw'] = dep_date_end_lw

        # Getting the yield_changes_rolling trigger parameters from
        # JUP_DB_Trigger_Types collection in dB
        self.get_trigger_details(trigger_name='yield_changes_rolling', db=db)

        for tup1, tup2 in zip(yield_data_tw.sort_values(by='market').iterrows(),
                              yield_data_lw.sort_values(by='market').iterrows()):
            try:
            ...
            #   Check whether the trigger is fired or not
                trigger_occurence = self.check_trigger(yield_data_tw=tup1[1],
                                                       yield_data_lw=tup2[1])
                ...
                YRT_TRIGGERS.append(id)
            except Exception as e:
                db.JUP_DB_Errors.insert({"err_id": "YIRL",
    ...
    # @staticmethod
    # @measure(JUPITER_LOGGER)
    # def get_distance(origin, destination, unit):
    #     """
    #
    #     :param unit:
    #     :param origin:
    ...
    #     :return:
    #     """
    #     ## Get the distance b/w OD
    #     return 500

    @measure(JUPITER_LOGGER)
    def check_trigger(self, yield_data_tw, yield_data_lw):
        """
        Change Parameter is calculated and checked with Thresholds
        Arguments:
        :param yield_data_tw: dict with ty and ly yield values as key value pairs
        :param yield_data_lw: dict with ty and ly yield values as key value pairs
        :returns a bool True if trigger is raised
                        False if trigger is not raised
        """
        self.old_doc_data['yield'] = yield_data_lw['yield_ty']
        self.new_doc_data['yield'] = yield_data_tw['yield_ty']
        # print self.old_doc_data['yield'], self.new_doc_data['yield']

        if self.old_doc_data['yield'] != 0 and self.old_doc_data['yield'] != "NA" and self.new_doc_data['yield'] != "NA":
            if self.threshold_type == 'percent':
                self.change = (
                    self.new_doc_data['yield'] - self.old_doc_data['yield']) * 100 / self.old_doc_data['yield']
            else:
                self.change = (
                    self.new_doc_data['yield'] -
            ...
            if self.change < self.lower_threshold or self.change > self.upper_threshold:
                ...
                return False
        elif (self.old_doc_data['yield'] == 0) and (self.new_doc_data['yield'] != 0) and self.old_doc_data['yield'] != "NA" and self.new_doc_data['yield'] != "NA":
            self.change = "inf"
            return True
        else:
            return False
...
def main_helper(db):
    ...
    for mrkt in ['UAETBSDXBY']:
        if counter == 2000:
            id_list = generate_yield_changes_rolling_triggers(data_list=markets,
                                                                pos=data['pos'],
                                                                origin=data['origin'],
