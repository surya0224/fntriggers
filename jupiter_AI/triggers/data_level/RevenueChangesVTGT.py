"""
File Name              :   RevenueChangesVTGT.py
Author                 :   Sai Krishna
Date Created           :   2016-02-07
Description            :   Generation of a RevenueChangesVTGT trigger for a market
    ...
    Let Observation Date - '2017-02-15'
        Parameters in Comparision -
            ...
            The following parameter are calculated.
                1.  Revenue for the month of departure attained till date.
                2.  Target Pax for the month of departure.
                3.  Last Year Revenue till Observation date for the departure month in consideration.
...
@measure(JUPITER_LOGGER)
def generate_revenue_changes_vtgt_triggers(
    ...
    try:
        ...
        if sig_flag:
            ...
            trigger_obj1 = RevenueChangesVTGT(data, SYSTEM_DATE)
            ...
            trigger_obj2 = RevenueChangesVTGT(data, SYSTEM_DATE)
            ...
            trigger_obj3 = RevenueChangesVTGT(data, SYSTEM_DATE)
            ...
            trigger_obj4 = RevenueChangesVTGT(data, SYSTEM_DATE)
        ...
        obj_error = error_class.ErrorObject(
            ...
            'jupter_AI/triggers/data_level/RevenueChangesVTGT.py method: generate_revenue_changes_vtgt_triggers',
            get_arg_lists(
                inspect.currentframe()))
        obj_error.append_to_error_list(str(error_msg))
        obj_error.write_error_logs(datetime.datetime.now())
    return RVTGT_TRIGGERS


class RevenueChangesVTGT(DataLevel):
    """
    Class Object that defines Revenue Changes VTGT Trigger
    """

    @measure(JUPITER_LOGGER)
    def __init__(self, data, system_date):
        """
        Initisating Function for the class
        Arguments:
            :data: dict with the following keys pos, origin, destination, compartment
            :system date: date of observation or consideration
        """
        super(RevenueChangesVTGT, self).__init__(data, system_date)
        self.old_doc_data = deepcopy(data)
        self.new_doc_data = deepcopy(data)
        self.trigger_date = system_date

    @measure(JUPITER_LOGGER)
    def do_analysis(self, dep_date_start, dep_date_end, db, data_list, is_event_trigger=False):
        """
        ...
        """
        #   Getting Forecast Rev data
        forecast_rev_val = self.get_forecast_values(
        ...
        #   Getting the Target Rev for the set of departure dates current year
        flown_rev_target_val = self.get_target_data(
            dep_date_start=dep_date_start,
            dep_date_end=dep_date_end,
            data_list=data_list,
            db=db,
            parameter='revenue')
        # print 'flown_rev_target'
        # print flown_rev_target_val.head()

        self.get_trigger_details(trigger_name='revenue_changes_VTGT', db=db)
        forecast_rev_val.sort_values(by='market', inplace=True)
        flown_rev_target_val.sort_values(by='market', inplace=True)
        rev_data = forecast_rev_val.merge(flown_rev_target_val, on='market', how='left')
        rev_data.dropna(inplace=True)
        # print rev_data.to_string()
        #   Check whether the trigger is fired or not
        for tup1 in rev_data.iterrows():
            try:
                ...
                # self.new_doc_data['forecast_revenue'] = tup1[1]['forecast_revenue']
                # self.old_doc_data['target_revenue'] = tup1[1]['target_revenue']
                trigger_occurence = self.check_trigger(
                    forecast_rev=tup1[1]['forecast_revenue'],
                    target_rev=tup1[1]['target_revenue'])
                ...
                RVTGT_TRIGGERS.append(id)
            except Exception as e:
                db.JUP_DB_Errors.insert({"err_id": "RVVT",
    ...
    def do_analysis_old(self, dep_date_start, dep_date_end, db, data_list, is_event_trigger=False):
        ...
        #   Getting Revenue data till today for this year and last year
        ...
        flown_revenue_target_val = self.get_target_data(
            dep_date_start=dep_date_start,
            dep_date_end=dep_date_end,
            parameter='revenue',
            db=db,
            data_list=data_list)
        # print 'flown_revenue_target'
        # print flown_revenue_target_val.head()

        #   Getting Actual Flown pax for the departure period last year
        flown_revenue_ly_val = self.get_sales_flown_data(
            ...
            parameter='revenue',
            data_list=data_list)
        # print 'flown revenue last year val'
        # print flown_revenue_ly_val.head()
        revenue_data.sort_values(by='market', inplace=True)
        flown_revenue_target_val.sort_values(by='market', inplace=True)
        flown_revenue_ly_val.sort_values(by='market', inplace=True)
        revenue_data = revenue_data.merge(flown_revenue_target_val, on='market', how='left')
        flown_revenue_ly_val = flown_revenue_ly_val.rename(columns={"ty": "flown_revenue_ty", "ly":"flown_revenue_ly"})
        revenue_data = revenue_data.merge(flown_revenue_ly_val, on='market', how='left')
        ...
        for tup1 in revenue_data.iterrows():
            try:
                ...
                trigger_occurence = self.check_trigger(
                    ...
                    flown_revenue_val=tup1[1]['flown_revenue_ty'],
                    flown_revenue_target_val=tup1[1]['target_revenue'])
                # print 'occurence', trigger_occurence
    ...
    @measure(JUPITER_LOGGER)
    def check_trigger_old(
            self,
            revenue_ly,
            revenue_ty,
            flown_revenue_val,
            flown_revenue_target_val):
        self.old_doc_data['revenue'] = revenue_ly
        self.old_doc_data['target'] = flown_revenue_val
        self.new_doc_data['revenue'] = revenue_ty
        self.new_doc_data['target'] = flown_revenue_target_val
        # print self.old_doc_data['revenue'], self.old_doc_data['target'], self.new_doc_data['revenue'], self.new_doc_data['target']
        if self.old_doc_data['target'] != 0 and self.new_doc_data['target'] != 0:
            self.old_doc_data['percent_acheived'] = self.old_doc_data['revenue'] / \
                self.old_doc_data['target']
            self.new_doc_data['percent_acheived'] = self.new_doc_data['revenue'] / \
            ...
            if self.old_doc_data['percent_acheived'] != 0:
                ...
                # print self.new_doc_data['percent_acheived'], self.old_doc_data['percent_acheived']
                ...
                if self.change < self.lower_threshold or self.change > self.upper_threshold:
                    ...
                    return False
            elif self.old_doc_data['percent_acheived'] == 0 and self.new_doc_data['percent_acheived'] != 0:
                self.change = 'inf'
                return True
            else:
                return False
    ...
    @measure(JUPITER_LOGGER)
    def check_trigger(self, forecast_rev, target_rev):
        self.old_doc_data['target_revenue'] = target_rev
        self.new_doc_data['forecast_revenue'] = forecast_rev
        # print self.old_doc_data['target_pax'], self.new_doc_data['forecast_pax']
        if self.old_doc_data['target_revenue'] != 0:

            if self.threshold_type == 'percent':
                self.change = (self.new_doc_data['forecast_revenue'] - self.old_doc_data[
                    'target_revenue']) * 100 / self.old_doc_data['target_revenue']
            else:
                self.change = (self.new_doc_data['forecast_revenue'] -
                               self.old_doc_data['target_revenue'])
            # print 'change', self.change
            if self.change < self.lower_threshold or self.change > self.upper_threshold:
                return True
            else:
...
def main_helper(db):
    ...
    for mrkt in ['CMBCMBDXBY']:
        if counter == 2000:
            id_list = generate_revenue_changes_vtgt_triggers(data_list=markets,
                                                                pos=data['pos'],
                                                                origin=data['origin'],
