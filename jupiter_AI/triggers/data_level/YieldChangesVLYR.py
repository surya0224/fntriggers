"""
File Name              :   YieldChangesVLYR.py
Author                 :   Sai Krishna
Date Created           :   2016-02-21
Description            :   Generation of a YieldChangesVLYR trigger for a market
...
Long Description for Trigger:
    Let Observation Date - '2017-02-15'
        Parameters in Comparision -
            ...
            2.  Yield obtained for departure dates b/w '2017-02-01' to '2017-02-28' till
...
@measure(JUPITER_LOGGER)
def generate_yield_changes_vlyr_triggers(
    ...
    try:
        ...
        if sig_flag:
            ...
            trigger_obj1 = YieldChangesVLYR(data, SYSTEM_DATE)
            ...
            trigger_obj2 = YieldChangesVLYR(data, SYSTEM_DATE)
            ...
            trigger_obj3 = YieldChangesVLYR(data, SYSTEM_DATE)
            ...
            trigger_obj4 = YieldChangesVLYR(data, SYSTEM_DATE)
        ...
        obj_error = error_class.ErrorObject(
            ...
            'jupter_AI/triggers/data_level/YieldChangesVLYR.py method: generate_yield_changes_vlyr_triggers',
            get_arg_lists(
...
class YieldChangesVLYR(DataLevel):
    @measure(JUPITER_LOGGER)
    def __init__(self, data, system_date):
        super(YieldChangesVLYR, self).__init__(data, system_date)
    ...
    def do_analysis(self, dep_date_start, dep_date_end, db, data_list, is_event_trigger=False):
        ...
        self.get_trigger_details(trigger_name='yield_changes_VLYR', db=db)
        for tup1 in yield_data.iterrows():
            try:
                ...
                YVL_TRIGGERS.append(id)
            except Exception as e:
                print "Error ---> "
                print traceback.print_exc()
                db.JUP_DB_Errors.insert({"err_id": "YIVL",
    ...
    @measure(JUPITER_LOGGER)
    def check_trigger(self, yield_ly, yield_ty):
        self.old_doc_data['yield'] = yield_ly
        # self.old_doc_data['capacity'] = capacity_ly
        self.new_doc_data['yield'] = yield_ty
        # self.new_doc_data['capacity'] = capacity_ty
        # print self.old_doc_data['yield'], self.new_doc_data['yield']
        if self.old_doc_data['yield'] > 0 and yield_ly != "NA" and yield_ty != "NA":
            ...
            #     # self.new_doc_data['yield_expected'] = math.ceil(
            #     #     self.new_doc_data['yield'] *
            #     #     self.old_doc_data['capacity'] /
            #     #     self.new_doc_data['capacity'])
            # else:
            #     # self.new_doc_data['yield_expected'] = self.old_doc_data['yield']
            ...
            if self.change < self.lower_threshold or self.change > self.upper_threshold:
                ...
                return False
        elif (self.old_doc_data['yield'] == 0 or self.old_doc_data['yield'] == "NA") and \
             (self.new_doc_data['yield'] == 0 or self.new_doc_data['yield'] != "NA"):
            self.change = 'inf'
            return True
        else:
            return False
...
def main_helper(db):
    ...
    for mrkt in list(sig_markets_df['market'].values):
        if counter == 2000:
            id_list = generate_yield_changes_vlyr_triggers(data_list=markets,
                                                                pos=data['pos'],
                                                                origin=data['origin'],
