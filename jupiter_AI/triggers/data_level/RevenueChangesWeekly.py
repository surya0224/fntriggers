"""
File Name              :   RevenueChangesWeekly.py
Author                 :   Sai Krishna
Date Created           :   2016-02-07
Description            :   Generation of a RevenueChangesWeekly trigger for a market
...
Long Description for Trigger:
    Let Observation Date - '2017-02-15'
        Parameters in Comparision -
            1.  Revenue obtained for departure dates b/w '2017-02-01' to '2017-02-28'(this month) this week
                '2017-02-08' to '2017-02-15
            2.  Revenue obtained for departure dates b/w '2017-01-01' to '2017-02-28' (this month) last week
...
@measure(JUPITER_LOGGER)
def generate_revenue_changes_weekly_triggers(pos,
                                             origin,
                                             destination,
                                             compartment,
                                             db,
    ...
    try:
        ...
        if sig_flag:
            ...
            trigger_obj1 = RevenueChangesWeekly(data, SYSTEM_DATE)
            ...
            trigger_obj2 = RevenueChangesWeekly(data, SYSTEM_DATE)
            ...
            trigger_obj3 = RevenueChangesWeekly(data, SYSTEM_DATE)
            ...
            trigger_obj4 = RevenueChangesWeekly(data, SYSTEM_DATE)
        ...
        obj_error = error_class.ErrorObject(
            ...
            'jupiter_AI/triggers/data_level/RevenueChangesWeekly.py method: generate_revenue_changes_weekly_triggers',
            get_arg_lists(
...
class RevenueChangesWeekly(DataLevel):
    """
    ...
    def __init__(self, data, system_date):
        super(RevenueChangesWeekly, self).__init__(data, system_date)
    ...
    def do_analysis(self, dep_date_start, dep_date_end, db, data_list, is_event_trigger=False):
        ...
        revenue_end_date_obj_tw = datetime.datetime.strptime(
            self.trigger_date,
            '%Y-%m-%d'
        )
        revenue_end_date_tw = self.trigger_date
        revenue_start_date_tw = datetime.datetime.strftime(
            revenue_end_date_obj_tw + datetime.timedelta(days=-7), '%Y-%m-%d')

        #   Getting Revenue data till today
        revenue_data_tw = self.get_sales_flown_data(
            book_date_start=revenue_start_date_tw,
            book_date_end=revenue_end_date_tw,
        ...
        revenue_end_date_obj_lw = revenue_end_date_obj_tw + \
            datetime.timedelta(days=-7)
        revenue_end_date_lw = datetime.datetime.strftime(
            revenue_end_date_obj_lw, '%Y-%m-%d')
        revenue_start_date_lw = datetime.datetime.strftime(
            revenue_end_date_obj_lw + datetime.timedelta(days=-7), '%Y-%m-%d')

        #   Obtaining revenue data for last weeks parameters
        revenue_data_lw = self.get_sales_flown_data(
            book_date_start=revenue_start_date_lw,
            book_date_end=revenue_end_date_lw,
            dep_date_start=dep_date_start,
            dep_date_end=dep_date_end,
            parameter='revenue',
            db=db,
        ...
        self.get_trigger_details(trigger_name='revenue_changes_weekly', db=db)
        ...
        for tup1, tup2 in zip(revenue_data_tw.sort_values(by='market').iterrows(),
            ...
            try:
                ...
                RWK_TRIGGERS.append(id)
            except Exception as e:
                db.JUP_DB_Errors.insert({"err_id": "RVWK",
...
def main_helper(db):
    ...
    for mrkt in list(sig_markets_df['market'].values):
        if counter == 2000:
            id_list = generate_revenue_changes_weekly_triggers(data_list=markets,
                                                                pos=data['pos'],
                                                                origin=data['origin'],
