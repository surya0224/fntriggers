"""
File Name              :   PaxChangesRolling.py
Author                 :   Sai Krishna
Date Created           :   2016-02-07
Description            :   Generation of a PaxChangesRolling trigger for a market
...
Long Description for Trigger:
    Let Observation Date - '2017-02-15'
        Parameters in Comparision -
            ...
            2.  Pax obtained for departure dates b/w '2017-01-23' to '2017-02-23' till
                '2017-02-15'.
...
@measure(JUPITER_LOGGER)
def generate_pax_changes_rolling_triggers(pos,
                                          origin,
                                          destination,
                                          compartment,
                                          db,
    ...
    try:
        ...
        if sig_flag:
            ...
            if len(dates_list) > 0:
                for date in dates_list:
                    trigger_obj = PaxChangesRolling(data, SYSTEM_DATE)
            ...
            trigger_obj1 = PaxChangesRolling(data, SYSTEM_DATE)
            ...
            trigger_obj2 = PaxChangesRolling(data, SYSTEM_DATE)
            ...
            trigger_obj3 = PaxChangesRolling(data, SYSTEM_DATE)
            ...
            trigger_obj4 = PaxChangesRolling(data, SYSTEM_DATE)
        ...
        obj_error = error_class.ErrorObject(
            ...
            'jupiter_AI/triggers/data_level/PaxChangesRolling.py method: generate_pax_changes_rolling_triggers',
            get_arg_lists(
                inspect.currentframe()))
        obj_error.append_to_error_list(str(error_msg))
        obj_error.write_error_logs(datetime.datetime.now())

    return PXRL_TRIGGERS

class PaxChangesRolling(DataLevel):
    """
    ...
    def __init__(self, data, system_date):
        super(PaxChangesRolling, self).__init__(data, system_date)
    ...
    def do_analysis(self, dep_date_start, dep_date_end, data_list, db, is_event_trigger=False):
        ...
        self.triggering_data.update({
            ...
            'book_date_end_lw': book_start_date_lw,
        ...
        for tup1, tup2 in zip(pax_data_tw.sort_values(by='market').iterrows(),
            ...
            # JUP_DB_Trigger_Types collection in dB
                self.get_trigger_details(trigger_name='pax_changes_rolling', db=db)
                ...
                PXRL_TRIGGERS.append(id)
            except Exception as e:
                db.JUP_DB_Errors.insert({"err_id": "PXRL",
    ...
    def check_trigger(self, pax_data_tw, pax_data_lw):
        ...
        # print self.old_doc_data['pax'], self.new_doc_data['pax']
        if self.old_doc_data['pax'] != 0 and self.new_doc_data['pax']!="NA" and self.old_doc_data['pax']!="NA":
            ...
            if self.change < self.lower_threshold or self.change > self.upper_threshold:
                ...
                return False
        elif self.old_doc_data['pax'] == 0 and self.new_doc_data['pax'] != 0 and self.new_doc_data['pax']!="NA" and self.old_doc_data['pax']!="NA":
            self.change = 'inf'
            return True
        else:
            return False
...
def main_helper(db):
    ...
    for mrkt in list(sig_markets_df['market'].values):
        if counter == 2000:
            id_list = generate_pax_changes_rolling_triggers(data_list=markets,
                                                                pos=data['pos'],
                                                                origin=data['origin'],
