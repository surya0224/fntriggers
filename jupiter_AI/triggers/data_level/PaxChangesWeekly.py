"""
File Name              :   PaxChangesWeekly.py
Author                 :   Sai Krishna
Date Created           :   2016-02-07
Description            :   Generation of a PaxChangesWeekly trigger for a market
...
Long Description for Trigger:
    Let Observation Date - '2017-02-15'
        Parameters in Comparision -
            1.  Pax obtained for departure dates b/w '2017-02-01' to '2017-02-28'(this month) till
                '2017-02-08' to '2017-02-15'(observation date)
            2.  Pax obtained for departure dates b/w '2017-01-01' to '2017-02-28' till
                '2017-02-01' to '2017-02-08'(observation date)
...
@measure(JUPITER_LOGGER)
def generate_pax_changes_weekly_triggers(pos,
                                         origin,
                                         destination,
                                         compartment,
                                         db,
    ...
    try:
        ...
        #   considering the default pax period Year to Observation Date
        ...
        if sig_flag:
            ...
            trigger_obj1 = PaxChangesWeekly(data, SYSTEM_DATE)
            ...
            trigger_obj2 = PaxChangesWeekly(data, SYSTEM_DATE)
            ...
            trigger_obj3 = PaxChangesWeekly(data, SYSTEM_DATE)
            ...
            trigger_obj4 = PaxChangesWeekly(data, SYSTEM_DATE)
        ...
        obj_error = error_class.ErrorObject(
            ...
            'jupiter_AI/triggers/data_level/PaxChangesWeekly.py method: generate_pax_changes_weekly_triggers',
            get_arg_lists(
                inspect.currentframe()))
        obj_error.append_to_error_list(str(error_msg))
        obj_error.write_error_logs(datetime.datetime.now())
    return PXWK_TRIGGERS

class PaxChangesWeekly(DataLevel):
    """
    Class object that defines Pax Changes Rolling Trigger
    Arguments:
    ...
    def __init__(self, data, system_date):
        super(PaxChangesWeekly, self).__init__(data, system_date)
    ...
    def do_analysis(self, dep_date_start, dep_date_end, data_list, db, is_event_trigger=False):
        ...
        """
        #   Obtaining pax period consideration till Obs date
        ...
        #   Getting Pax data till today
        pax_data_tw = self.get_sales_flown_data(
            ...
            data_list=data_list)
        # print 'pax_tw'
        # print pax_data_tw.head()

        #   Triggering data represents departure date ranges and pax date ranges under consideration
        ...
        #   pax start date lw = year start date
        #   pax end date lw = pax end date this week(obs date) - 7 days
        ...
        #   Obtaining pax data for last weeks parameters
        pax_data_lw = self.get_sales_flown_data(
        ...
        # Getting the pax_changes_rolling trigger parameters from
        # JUP_DB_Trigger_Types collection in dB
        self.get_trigger_details(trigger_name='pax_changes_weekly', db=db)
        for tup1, tup2 in zip(pax_data_tw.sort_values(by='market').iterrows(),
                              pax_data_lw.sort_values(by='market').iterrows()):
            try:
                ...
                trigger_occurence = self.check_trigger(pax_data_tw=tup1[1],
                                                       pax_data_lw=tup2[1])
                ...
                PXWK_TRIGGERS.append(id)
            except Exception as e:
                db.JUP_DB_Errors.insert({"err_id": "PXWK",
    ...
    @measure(JUPITER_LOGGER)
    def check_trigger(self, pax_data_tw, pax_data_lw):
        """
        Change Parameter is calculated and checked with Thresholds
        Arguments:
        :param pax_data_tw: dict with ty and ly pax values as key value pairs
        :param pax_data_lw: dict with ty and ly pax values as key value pairs
        :returns a bool True if trigger is raised
                        False if trigger is not raised
        """
        self.old_doc_data['pax'] = pax_data_lw['ty']
        self.new_doc_data['pax'] = pax_data_tw['ty']
        if self.old_doc_data['pax'] != 0 and self.old_doc_data['pax'] != "NA" and self.new_doc_data['pax'] != "NA":
            if self.threshold_type == 'percent':
                self.change = (
                    self.new_doc_data['pax'] - self.old_doc_data['pax']) * 100 / self.old_doc_data['pax']
            else:
                self.change = (
                    self.new_doc_data['pax'] -
                    self.old_doc_data['pax'])
            # print 'change', self.change
            if self.change < self.lower_threshold or self.change > self.upper_threshold:
                return True
            else:
                return False
        elif self.old_doc_data['pax'] == 0 and self.new_doc_data['pax'] != 0 and self.old_doc_data['pax'] != "NA" and self.new_doc_data['pax'] != "NA":
            self.change = 'inf'
            return True
        else:
            return False
...
def main_helper(db):
    ...
    for mrkt in list(sig_markets_df['market'].values):
        if counter == 2000:
            id_list = generate_pax_changes_weekly_triggers(data_list=markets,
                                                                pos=data['pos'],
                                                                origin=data['origin'],
                                                                db=db,
