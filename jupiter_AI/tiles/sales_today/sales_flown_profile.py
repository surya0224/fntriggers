"""
File Name              :   sales_flown_profile.py
Author                 :   Pavan
Date Created           :   2016-12-19
Description            :   module with methods to calculate net sales flown for jupiter dashboard (sales today)
MODIFICATIONS LOG      :
    S.No               :
    Date Modified      :
    By                 :
...
def get_sales_profile_flown(afilter):
    ...
    if (data['origin'] != [] and data['destination'] != []):
        ...
        sales_flown_pipeline = db['JUP_DB_Sales_Flown'].aggregate([
                                               {
                                                    '$match': query
                                               }
                                               ,
        ...
        sales_flown_pipeline = db['JUP_DB_Cumulative_Dep_Date'].aggregate([
                                                             {
                                                                '$match': query
                                                             }
                                                             ,
    ...
    net_sales_flown_revenue = 0
    sales_flown_pipeline_list = list(sales_flown_pipeline)
    #print sales_flown_pipeline_list
    if len(sales_flown_pipeline_list) != 0:
        for i in sales_flown_pipeline_list:
            net_sales_flown_revenue += i['revenue']
        return net_sales_flown_revenue
    # else:
...
filter = {'region': ['GCC'],
     ...
     'fromDate': '2016-07-10',
     'toDate': '2016-07-12'}
result = get_sales_profile_flown(filter)
#print result
# print time.time() - st
