"""
File Name              :   market_share_new_entrant_perspective_BLL
Author                 :   Ashwin Kumar
...
import jupiter_AI.RnA.market_share_new_entrant_perspective_DAL as dal
...
def rna_market_share_price(dict_scr_filter):
    ...
    ******** please enter codes to do the above ***********
    ...
    rna_response = dict()
    capacity_collection = dal.get_new_entrant_related_data(dict_scr_filter)
    ...
    if len(capacity_collection) == 0:
        ...
        action = 'Do not react if current market share is above fair market share and if the ' \
                 'ticketed to capacity ratio is more than 90 %. If not float  an appropriate fare' \
                 ' in a RBD with appropriate inventory with a firm end date.'
