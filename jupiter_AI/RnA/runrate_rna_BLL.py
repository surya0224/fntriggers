...
import jupiter_AI.RnA.runrate_rna_DAL as dal
...
def check_runrate(dict_scr_filter):
    """
...
def rna_runrate(dict_scr_filter):

    """
    the function rna_runrate will be called to check the value of the bookings.
    ...
    ******** please enter codes to do the above ***********
    ...
    rna_response = dict()
    runrate_collection_name = dal.get_runrate_rna_data(dict_scr_filter)
    ...
    if len(runrate_collection_name) == 0:
        ...
        why = '(Runrate is low )Pricing model is not good compare to competitors'
    ...
    elif len(runrate_collection_name) == 1:
