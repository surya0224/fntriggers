"""
File Name              :   Bookings_capacity_host_BLL
...
import jupiter_AI.RnA.Bookings_capacity_host_DAL as dal
...
def check_bookings_capacity(filter):
    """
...
def rna_bookings_capacity(filter):
    ...
    ******** please enter codes to do the above ***********
    ...
    rna_response = dict()
    bookings_capacity_collection_name = dal.get_bookings_capacity_rna_data(filter)
    check_for_bookings = cf.get_btc_data(filter)
    ...
    if len(bookings_capacity_collection_name) == 0:
        ...
        why = 'Competition has  increased its capacity : The host has decreased its capacity'
    ...
    elif len(bookings_capacity_collection_name) == 1:
