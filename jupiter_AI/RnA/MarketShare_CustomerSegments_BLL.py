...
"""
File Name              :   MarketShare_CustomerSegments_BLL
...
import jupiter_AI.RnA.MarketShare_CustomerSegments_DAL as DAL
...
def rna_customer_segments(dict_scr_filter):
    ...
    ******** please enter codes to do the above ***********
    ...
    rna_response = dict()
    customer_segments_collection = DAL.get_customer_segments_data(dict_scr_filter)
    ...
    if len(customer_segments_collection) == 0:
        ...
        what = 'Markets Share - Customer Segments'
        why = 'There are special promotions directed towards specific customer segments on the web'
        status_quo = 'Further drop in market share'
        action = 'Do not react if current market share is above fair market share. If not, match promotions on' \
                 'the web if you have capacity. Set up a rule in ATPCO diffuser.'
    elif len(customer_segments_collection) == 1:
