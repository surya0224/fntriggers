"""
File Name              :	cust_seg_specific_fares
Author                 :	K Sai Krishna
Date Created           :	2016-12-19
Description            :	Business Logic for the
                            RnA analysis for review of specific_fares for different customer segments
...
def gen_customer_segments_fares_rna(dict_scr_filter):
    ...
    if len(data) == 0:
        ...
        why = ''.join(['Review the specific fares quoted for different Customer Segments'])
        status_quo = ''.join([''])
        action = ''.join([''])
    elif len(data) == 1:
