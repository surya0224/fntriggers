...
import jupiter_AI.RnA.Distribution_DAL as dal
...
def check_bookings_distribution(dict_scr_filter):
    """
...
def rna_bookings_distribution(dict_scr_filter):

    """
    the function check_bookings_distribution will be called to check the value of the bookings.

    ******** please enter codes to do the above ***********
    """

    rna_response = dict()
    bookings_distribution_collection_name = dal.get_bookings_distribution_rna_data(dict_scr_filter)
    ...
    if len(bookings_distribution_collection_name) == 0:
        ...
        # raise no_data_error
        #As for now displaying data

        what = 'Bookings - Distribution'
        why = 'There are special fare given to consolidators or GSAs that are not in the published world,BTF indicator has deteriorated'
        status_quo = 'Further drop in market share'
        action = 'Decide if it is okay to offer a special deal to set of distributors,Use ATPCO diffuser to distribute private fares, Not an issue if we have more than 90% of the capacity ticketed.'

    elif len(bookings_distribution_collection_name) == 1:
...
if __name__ == '__main__':
    ...
    print rna_bookings_distribution (a)