...
File Name: market_distributors.py

Creates distributor characteristics from sales and flown data.
...
def run_this_year(snap_date, db):
    ...
    try:
        print 'Merging'
        temp3 = pd.merge(df_1, df_2, on=['od', 'pos', 'compartment', 'distributor', 'origin', 'destination',
        ...
        distributor = temp3.to_dict("records")
        print 'Inserting'
        db.JUP_DB_Market_Characteristics_Distributors.insert(distributor)
        del temp3
    except KeyError:
        if len(df_1) == 0 and len(df_2) != 0:
            distributor = df_2
            distributor['dep_month'] = distributor['dep_date'].str.slice(5, 7).astype('int')
            distributor['dep_year'] = distributor['dep_date'].str.slice(0, 4).astype('int')
            distributor['snap_month'] = distributor['snap_date'].str.slice(5, 7).astype('int')
            distributor['snap_year'] = distributor['snap_date'].str.slice(0, 4).astype('int')
            distributor = distributor.to_dict('records')
            print 'Inserting'
            db.JUP_DB_Market_Characteristics_Distributors.insert(distributor)
    ...
    del df_2
    del distributor
    print 'Inserted Distributors This Year for snap:', snap_date
...
def run_cap_and_last_year(snap_date, db):
    count = 1
    counti = 1
    cur_market_distributor = db.JUP_DB_Market_Characteristics_Distributors.find({"snap_date": {"$eq": snap_date}}, no_cursor_timeout=True)
    for k in cur_market_distributor:
        try:
            if k['sales'] == -999:
        ...
        last_year_dep_date = datetime.strftime(datetime.strptime(k['dep_date'], "%Y-%m-%d") -
                                               timedelta(days=364), "%Y-%m-%d")
        last_year = db.JUP_DB_Market_Characteristics_Distributors.find({"snap_date": last_year_snap_date, "dep_date": last_year_dep_date, "od": k["od"],
        ...
        for j in last_year:
            ...
            print count, "      ",k['_id']
            count += 1
        db.JUP_DB_Market_Characteristics_Distributors.update({"_id": k["_id"]}, k)
        if counti % 1000 == 0:
            print 'Updated:', counti
        counti += 1
    cur_market_distributor.close()


@measure(JUPITER_LOGGER)
def run_market_distributors(client):
    db = client[JUPITER_DB]
    snap_date = SYSTEM_DATE
    st = time.time()
    print 'Removing present snaps in case of re run'
    db.JUP_DB_Market_Characteristics_Distributors.remove({'snap_date': snap_date})
    print 'Removed in ', time.time() - st
    print 'Running Distributors This Year for snap:', snap_date
    run_this_year(snap_date=snap_date, db=db)
    print 'Time taken for this year distributors', time.time() - st
    print 'Runing Distributors Last Year, Capacity, Target, Forecast'
    run_cap_and_last_year(snap_date=snap_date, db=db)
    print 'Ran last year, capacity, target, forecast for snap:', snap_date
    print 'Total Time Taken Distributors', time.time() - st


@measure(JUPITER_LOGGER)
def market_distributors_adhoc(snap_date):
    st = time.time()
    print 'Running Distributors This Year for snap:', snap_date
...
if __name__ == "__main__":
    client = mongo_client()
    run_market_distributors(client)
    client.close()