...
File Name: market_segments.py

Creates segment characteristics from sales and flown data.
...
def run_this_year(snap_date, db):
    ...
    cur_sales = db.JUP_DB_Sales.aggregate([
        {'$match': {
            '$or': [{"snap_date": snap_date, 'segment_fz': {'$ne': None}},
                    {"enrole_update_date": snap_date, 'segment_fz': {'$ne': None}}
                    ]
        }},
        {
    ...
    try:
        print 'Merging'
        temp3 = pd.merge(df_1, df_2, on=['od', 'pos', 'compartment', 'segment', 'origin', 'destination',
        ...
        segment = temp3.to_dict("records")
        print 'Inserting'
        db.JUP_DB_Market_Characteristics_Segments.insert(segment)
        del temp3
    except KeyError:
        if len(df_1) == 0 and len(df_2) != 0:
            segment = df_2
            segment['dep_month'] = segment['dep_date'].str.slice(5, 7).astype('int')
            segment['dep_year'] = segment['dep_date'].str.slice(0, 4).astype('int')
            segment['snap_month'] = segment['snap_date'].str.slice(5, 7).astype('int')
            segment['snap_year'] = segment['snap_date'].str.slice(0, 4).astype('int')
    ...
    del df_2
    del segment
    print 'Inserted segments This Year for snap:', snap_date
...
def run_cap_and_last_year(snap_date, db):
    count = 1
    counti = 1
    cur_market_segment = db.JUP_DB_Market_Characteristics_Segments.find({"snap_date": {"$eq": snap_date}}, no_cursor_timeout=True)
    for k in cur_market_segment:
        try:
            if k['sales'] == -999:
        ...
        last_year_dep_date = datetime.strftime(datetime.strptime(k['dep_date'], "%Y-%m-%d") -
                                               timedelta(days=364), "%Y-%m-%d")
        last_year = db.JUP_DB_Market_Characteristics_Segments.find({"snap_date": last_year_snap_date, "dep_date": last_year_dep_date, "od": k["od"],
        ...
        for j in last_year:
            ...
            print count, "      ",k['_id']
            count += 1
        db.JUP_DB_Market_Characteristics_Segments.update({"_id": k["_id"]}, k)
        if counti % 1000 == 0:
            print 'Updated:', counti
        counti += 1
    cur_market_segment.close()


@measure(JUPITER_LOGGER)
def run_market_segments(client):
    db = client[JUPITER_DB]
    snap_date = SYSTEM_DATE
    st = time.time()
    print "Updating segment_fz in sales and flown"
    Segment_FZ.run()
    print 'Removing present snaps in case of re run'
    db.JUP_DB_Market_Characteristics_Segments.remove({'snap_date': snap_date})
    print 'Removed in ', time.time() - st
    print 'Running segments This Year for snap:', snap_date
    run_this_year(snap_date=snap_date, db=db)
    print 'Time taken for this year segments', time.time() - st
    print 'Runing segments Last Year, Capacity, Target, Forecast'
    run_cap_and_last_year(snap_date=snap_date, db=db)
    print 'Ran last year, capacity, target, forecast for snap:', snap_date
    print 'Total Time Taken segments', time.time() - st


@measure(JUPITER_LOGGER)
def market_segments_adhoc(snap_date):
    st = time.time()
    print 'Running segments This Year for snap:', snap_date
