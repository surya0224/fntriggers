...
File Name: market_farebrand.py

Creates farebrand characteristics from sales and flown data.
...
def farebrand_check(farebasis):
    try:
        farebrand_type = re.search('[6-9]', farebasis).group()
        if farebrand_type == '6':
            return "Pay To Change"
        elif farebrand_type == '7':
            return "Free To Change"
        elif farebrand_type == '8':
            return 'Special Offer'
        elif farebrand_type == '9':
            return 'Business'
...
@measure(JUPITER_LOGGER)
def update_farebrands(snap_date, db):
    cur_sales = db.JUP_DB_Sales.find({"fare_brand": None, "channel": {"$nin": ["GDS"]}, "snap_date": {"$eq": snap_date}})
    bulk_list = []
    count = 1
    t = 0
    for i in cur_sales:
        fare_brand = farebrand_check(i["fare_basis"])
        if t == 10000:
            print "updating: ", count
            db.JUP_DB_Sales.bulk_write(bulk_list)
            bulk_list = []
            bulk_list.append(UpdateOne({"_id": i["_id"]}, {"$set": {"fare_brand": fare_brand}}))
            t = 1
        else:
            bulk_list.append(UpdateOne({"_id": i["_id"]}, {"$set": {"fare_brand": fare_brand}}))
            t += 1
    ...
    cur_flown = db.JUP_DB_Sales_Flown.find({"fare_brand": None, "channel": {"$nin": ["GDS"]}, "snap_date": {"$eq": snap_date}})
    bulk_list = []
    count = 1
    t = 0
    for i in cur_flown:
        ...
        if t == 10000:
            print "updating: ", count
            db.JUP_DB_Sales_Flown.bulk_write(bulk_list)
            bulk_list = []
            bulk_list.append(UpdateOne({"_id": i["_id"]}, {"$set": {"fare_brand": fare_brand}}))
            t = 1
        else:
...
def run_this_year(snap_date, db):
    ...
    try:
        print 'Merging'
        temp3 = pd.merge(df_1, df_2, on=['od', 'pos', 'compartment', 'farebrand', 'origin', 'destination',
        ...
        farebrand = temp3.to_dict("records")
        print 'Inserting'
        db.JUP_DB_Market_Characteristics_FareBrands.insert(farebrand)
        del temp3
    except KeyError:
        if len(df_1) == 0 and len(df_2) != 0:
            farebrand = df_2
            farebrand['dep_month'] = farebrand['dep_date'].str.slice(5, 7).astype('int')
            farebrand['dep_year'] = farebrand['dep_date'].str.slice(0, 4).astype('int')
            farebrand['snap_month'] = farebrand['snap_date'].str.slice(5, 7).astype('int')
            farebrand['snap_year'] = farebrand['snap_date'].str.slice(0, 4).astype('int')
            farebrand = farebrand.to_dict('records')
            print 'Inserting'
            db.JUP_DB_Market_Characteristics_FareBrands.insert(farebrand)
    ...
    del df_2
    del farebrand
    print 'Inserted farebrands This Year for snap:', snap_date
...
def run_cap_and_last_year(snap_date, db):
    ...
    for k in cur_market_farebrand:
        ...
        last_year_dep_date = datetime.strftime(datetime.strptime(k['dep_date'], "%Y-%m-%d") -
                                               timedelta(days=364), "%Y-%m-%d")
        last_year = db.JUP_DB_Market_Characteristics_FareBrands.find({"snap_date": last_year_snap_date, "dep_date": last_year_dep_date, "od": k["od"],
                                             "pos": k["pos"], "compartment": k["compartment"],
                                             "farebrand": k["farebrand"]})
        for j in last_year:
            a = 0
            b = 0
            try:
            ...
            print count, "      ",k['_id']
            count += 1
        db.JUP_DB_Market_Characteristics_FareBrands.update({"_id": k["_id"]}, k)
        if counti % 1000 == 0:
            print 'Updated:', counti
        counti += 1
    cur_market_farebrand.close()


@measure(JUPITER_LOGGER)
def run_market_farebrand(client):
    db = client[JUPITER_DB]
    snap_date = SYSTEM_DATE
    st = time.time()
    # print 'Updating farebrand in sales ans flown for snap:', snap_date
    # update_farebrands(snap_date=snap_date)
    print 'Removing present snaps in case of re run'
    db.JUP_DB_Market_Characteristics_FareBrands.remove({'snap_date': snap_date})
    print 'Removed in ', time.time() - st
    print 'Running farebrands This Year for snap:', snap_date
    run_this_year(snap_date=snap_date, db=db)
    print 'Time taken for this year farebrands', time.time() - st
    print 'Runing farebrands Last Year, Capacity, Target, Forecast'
    run_cap_and_last_year(snap_date=snap_date, db=db)
    print 'Ran last year, capacity, target, forecast for snap:', snap_date
    print 'Total Time Taken farebrands', time.time() - st


@measure(JUPITER_LOGGER)
def market_farebrand_adhoc(snap_date):
    st = time.time()
    # print 'Updating farebrand in sales and flown for snap:', snap_date
    # update_farebrands(snap_date=snap_date)
    print 'Running farebrands This Year for snap:', snap_date
