...
File Name: market_agents.py

Creates agent characteristics from sales and flown data.
...
def run_this_year(snap_date, db):
    ...
    try:
        print 'Merging'
        temp3 = pd.merge(df_1, df_2, on=['od', 'pos', 'compartment', 'agent', 'origin', 'destination',
        ...
        agent = temp3.to_dict("records")
        print 'Inserting'
        db.JUP_DB_Market_Characteristics_Agents.insert(agent)
        del temp3
    except KeyError:
        if len(df_1) == 0 and len(df_2) != 0:
            agent = df_2
            agent['dep_month'] = agent['dep_date'].str.slice(5, 7).astype('int')
            agent['dep_year'] = agent['dep_date'].str.slice(0, 4).astype('int')
            agent['snap_month'] = agent['snap_date'].str.slice(5, 7).astype('int')
            agent['snap_year'] = agent['snap_date'].str.slice(0, 4).astype('int')
            agent = agent.to_dict('records')
            print 'Inserting'
            db.JUP_DB_Market_Characteristics_Agents.insert(agent)
    ...
    del df_2
    del agent
    print 'Inserted agents This Year for snap:', snap_date
...
def run_cap_and_last_year(snap_date, db):
    ...
    for k in cur_market_agent:
        ...
        last_year_dep_date = datetime.strftime(datetime.strptime(k['dep_date'], "%Y-%m-%d") -
                                               timedelta(days=364), "%Y-%m-%d")
        last_year = db.JUP_DB_Market_Characteristics_Agents.find({"snap_date": last_year_snap_date, "dep_date": last_year_dep_date, "od": k["od"],
        ...
        for j in last_year:
            ...
            print count, "      ",k['_id']
            count += 1
        db.JUP_DB_Market_Characteristics_Agents.update({"_id": k["_id"]}, k)
        if counti % 1000 == 0:
            print 'Updated:', counti
        counti += 1
    cur_market_agent.close()


@measure(JUPITER_LOGGER)
def run_market_agents(client):
    db = client[JUPITER_DB]
    snap_date = SYSTEM_DATE
    st = time.time()
    print 'Removing present snaps in case of re run'
    db.JUP_DB_Market_Characteristics_Agents.remove({'snap_date': snap_date})
    print 'Removed in ', time.time() - st
    print 'Running agents This Year for snap:', snap_date
    run_this_year(snap_date=snap_date, db=db)
    print 'Time taken for this year agents', time.time() - st
    print 'Runing agents Last Year, Capacity, Target, Forecast'
    run_cap_and_last_year(snap_date=snap_date, db=db)
    print 'Ran last year, capacity, target, forecast for snap:', snap_date
    print 'Total Time Taken agents', time.time() - st


@measure(JUPITER_LOGGER)
def market_agents_adhoc(snap_date):
    st = time.time()
    print 'Running agents This Year for snap:', snap_date
