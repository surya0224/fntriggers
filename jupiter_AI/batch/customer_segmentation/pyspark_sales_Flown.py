...
End date of developement: 2017-12-30
Code functionality:
     Updates collection JUP_DB_Sales_Flown with segments_amadeus using KMeans Clustering Technique.
     Uses base collections - Sales_Flown
...
def run():
    ...
    cursor.close()
    model_f = MiniBatchKMeans(n_clusters=6, random_state = 9999)
    ...
    for turns in range(1,loop_count):
        cursor = db.JUP_DB_Sales_Flown.find({},no_cursor_timeout=True).skip(skips).limit(limits)
        dep_date_list = []
        book_date_list = []
        month_list = []
        pax_list = []
        ...
        model_f.partial_fit(train)
        skips = skips + 1000000
        joblib.dump(model_f, 'mainmodel-SalesFlown.sav')
    ...
    skips = 0
    limits = 1000000
    # total_coll = conn.JUP_DB_Sales.find().count()
    # print total_coll
    # total_coll1 = total_coll/limits
    # print total_coll1
    seg_0 = [0, 0, 0, 0, 0, 0, 0]
    seg_1 = [0, 0, 0, 0, 0, 0, 0]
    seg_2 = [0, 0, 0, 0, 0, 0, 0]
    ...
    for turns in range(1, loop_count):
        ...
        try:
            ...
            df3 = df2[['Advance_Booking', 'Channel_GDS', 'Compartment_J', 'Compartment_Y', 'DOW', 'Duration', 'Pax',
                       'Duration_Index']]
            print(len(df3))
        model_f = joblib.load('mainmodel-SalesFlown.sav')
