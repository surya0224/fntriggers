...
File Name: zinc_flights.py

This code analyzes the zin flights that have been running with near full capacity for a significant period of
time and populates the JUP_DB_Zinc_Flights collection daily as a batch file.
...
def get_threshold_values(db):
    ...
    crsr = db.JUP_DB_Thresholds.find({"item": "Zinc Flights", "active": True,
...
@measure(JUPITER_LOGGER)
def get_zinc_flights_query(valid_start_date, valid_end_date, seat_factor_upper_threshold, seat_factor_lower_threshold):
    query_pipeline_y = [
        {
            '$match': {
                'dep_date': {
...
def add_user_defined_flights(user_defined_flights, db):
    for j in user_defined_flights:
        ...
        if len(inv_cur) > 0:
            ...
            db.JUP_DB_Zinc_Flights.insert(dict_to_insert)
        else:
            print 'No record found in Inventory leg'


@measure(JUPITER_LOGGER)
def generate_zinc_flights(client):
    ...
    result = db.JUP_DB_Zinc_Flights.delete_many({})
    print "removed", result.deleted_count
    query_pipeline_y, query_pipeline_j = get_zinc_flights_query(valid_start_date, valid_end_date,
    ...
    for i in cursor:
        if i['high_sf_percentage'] >= frequency:
            ...
            db.JUP_DB_Zinc_Flights.insert(i)
