...
File Name: fare_rules_top_comp_6.py
...
cur_fare_record = fare_record.find({"carrier":{"$in":["EY"]}}, timeout = False, allowDiskUse = True).limit(1)
...
for i in cur_fare_record:
    try:
        if counti%1000 == 0:
             print counti
        if (int(i["tariff_code"]) < 10):
            i["tariff_code"] = str(0) + str(0) + str(i["tariff_code"])
        elif (int(i["tariff_code"]) < 100 and int(i["tariff_code"]) > 9):
            i["tariff_code"] = str(0) + str(i["tariff_code"])
        else:
            i["tariff_code"] = str(i["tariff_code"])

        if (i["effective_to"] != "" and i["effective_to"] != None):
            i["effective_to"] = str(0) + datetime.datetime.strftime(datetime.datetime.strptime(i["effective_to"], "%Y-%m-%d"), "%y%m%d")
        else:
            i["effective_to"] = "0999999"

        if (i["effective_from"] != "" and i["effective_from"] != None):
            i["effective_from"] = str(0) + datetime.datetime.strftime(datetime.datetime.strptime(i["effective_from"], "%Y-%m-%d"), "%y%m%d")
        else:
        ...
        i["rtg"] = int(i["rtg"])
        if (int(i["rtg"]) < 10):
            i["rtg"] = str(0) + str(0) + str(0) + str(0) + str(i["rtg"])
        elif (int(i["rtg"]) < 100 and int(i["rtg"]) > 9):
            i["rtg"] = str(0) + str(0) + str(0) + str(i["rtg"])
        elif (int(i["rtg"]) < 1000 and int(i["rtg"]) > 99):
            i["rtg"] = str(0) + str(0) + str(i["rtg"])
        elif (int(i["rtg"]) < 10000 and int(i["rtg"]) > 999):
            i["rtg"] = str(0) + str(i["rtg"])
        else:
            i["rtg"] = str(i["rtg"])
        # print i["rtg"]

        i["oneway_return"] = str(i["oneway_return"])
        if (i["oneway_return"] == "3"):
            i["oneway_return"] = "1"

        if (i["footnote"] != "" and i["footnote"] != None):
            i["footnote"] = str(i["footnote"])
        else:
            i["footnote"] = ""

        # print "format check"
        # print i["effective_from"], " ", i["effective_to"]
        ...
        # print"record 1 queried"
        for j in cur_record_1:
            # print ".........",j["DATES_DISC"], j["DATES_EFF"]
            #                 j["id_FR"] = (i["_id"])
            #             j["rtg"] = i["rtg"]
            #             j["footnote"] = i["footnote"]
            #             j["origin"] = i["origin"]
            #             j["destination"] = i["destination"]
            # print "--------"
            i['BATCH_CI'] = j['BATCH_CI']
            i['LOC_1_COUNTRY'] = j['LOC_1_COUNTRY']
            # i['FCI_CARRIER_TBL_990_1'] = j['FCI_CARRIER_TBL_990_1']
            # i['FCI_DI_1'] = j['FCI_DI_1']
            i['TYPE_CODES_FARE_TYPE'] = j['TYPE_CODES_FARE_TYPE']
            # i['FCI_PSGR_AGE_MIN_1'] = j['FCI_PSGR_AGE_MIN_1']
            # i['FCI_TDM_1'] = j['FCI_TDM_1']
            i['FARE_CLASS_INFORMATION_SEGS'] = j['FARE_CLASS_INFORMATION_SEGS']
            # i['FCI_PSGR_AGE_MAX_1'] = j['FCI_PSGR_AGE_MAX_1']
            i['OW_RT'] = j['OW_RT']
            i['LOC_1_TYPE'] = j['LOC_1_TYPE']
            i['LOC_2_AREA'] = j['LOC_2_AREA']
            i['TEXT_TBL_NO'] = j['TEXT_TBL_NO']
            i['LOC_1_ZONE'] = j['LOC_1_ZONE']
            i['DATES_DISC'] = j['DATES_DISC']
            i['TARIFF'] = j['TARIFF']
            i['LOC_2_STATE'] = j['LOC_2_STATE']
            i['CXR_CODE'] = j['CXR_CODE']
            # i['FCI_TICKETING_CODE_1'] = j['FCI_TICKETING_CODE_1']
            i['FT_NT'] = j['FT_NT']
            i['LOC_2_CITY'] = j['LOC_2_CITY']
            # i['FCI_PSGR_TYPE_1'] = j['FCI_PSGR_TYPE_1']
            i['LOC_1_AREA'] = j['LOC_1_AREA']
            # i['FCI_FILLER_1'] = j['FCI_FILLER_1']
            i['ACTION'] = j['ACTION']
            i['DATES_DISC_1'] = j['DATES_DISC_1']
            # i['FCI_TCM_1'] = j['FCI_TCM_1']
            i['NO_SEGS'] = j['NO_SEGS']
            i['RTG_NO'] = j['RTG_NO']
            i['LOC_2_ZONE'] = j['LOC_2_ZONE']
            i['LOC_2_COUNTRY'] = j['LOC_2_COUNTRY']
            i['DATES_EFF'] = j['DATES_EFF']
            i['RULE_NO'] = j['RULE_NO']
            i['DATES_EFF_1'] = j['DATES_EFF_1']
            # i['FCI_RBD_TBL_994_1'] = j['FCI_RBD_TBL_994_1']
            # i['FCI_DATE_TBL_994_1'] = j['FCI_DATE_TBL_994_1']
            i['BATCH_NO'] = j['BATCH_NO']
            i['MCN'] = j['MCN']
            i['PRC_CAT'] = j['PRC_CAT']
            # i['FCI_TICKET_DESIGNATOR_1'] = j['FCI_TICKET_DESIGNATOR_1']
            i['DIS_CAT'] = j['DIS_CAT']
            i['LOC_1_STATE'] = j['LOC_1_STATE']
            i['FARE_CLASS'] = j['FARE_CLASS']
            # i['FCI_RBD_1'] = j['FCI_RBD_1']
            i['LOC_1_CITY'] = j['LOC_1_CITY']
            i['TYPE_CODES_DAY_OF_WEEK_TYPE'] = j['TYPE_CODES_DAY_OF_WEEK_TYPE']
            i['REC_TYPE'] = j['REC_TYPE']
            i['SEQ_NO'] = j['SEQ_NO']
            i['UNAVAIL'] = j['UNAVAIL']
            i['LOC_2_TYPE'] = j['LOC_2_TYPE']
            # i['_id'] = j['_id']
            i['LOC_2'] = j['LOC_2']
            i['TYPE_CODES_SEASON_TYPE'] = j['TYPE_CODES_SEASON_TYPE']
            # i['FCI_COMMERCIAL_NAME_1'] = j['FCI_COMMERCIAL_NAME_1']
            i['LOC_1'] = j['LOC_1']
...
# cur = fare_record.find({})
# print "stage 1 complete, length:",len(list(cur))
...
temp_cur = fare_record.find({"used":0, "carrier":{"$in":["QR"]}, "tariff_code":{"$in":[
'022',
'026',
'032',
'458',
...
for i in temp_cur:
    ...
    if (i["tariff_code"] != tariff_code or i["carrier"] != carrier):
        cur_record_0 = record_0.find({"TARIFF": {"$eq": i["tariff_code"]}, "CXR_CODE": {"$eq": i["carrier"]}})
        cur_record_0 = list(cur_record_0)
    ...
    for h in cur_record_0:
        ...
        for g in range(int(h["NO_SEGS"])):
            ...
            for j in cur_record_2_cat_all:
                ...
                if wild_card_check(i["FARE_CLASS"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "":
    ...
    for j in cur_record_2_cat_all:
        ...
        if ((j["CAT_NO"] not in cat_check) and (wild_card_check(i["FARE_CLASS"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "")):
    ...
    if (i["fare_basis"] != fare_basis or i["tariff_code"] != tariff_code or i["carrier"] != carrier or \
                    i["fare_rule"] != fare_rule or i["footnote"] != footnote):
        fare_basis = i["fare_basis"]
        tariff_code = i["tariff_code"]
        carrier = i["carrier"]
        fare_rule = i["fare_rule"]
        footnote = i["footnote"]
