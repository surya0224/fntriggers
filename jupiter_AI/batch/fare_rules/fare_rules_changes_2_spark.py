...
File Name: fare_rules_changes_2.py
...
client = MongoClient("mongodb://pdssETLUser:"+urllib.quote('pdssETL@2017')+"@172.28.23.5:27017/")
conf = SparkConf()
conf.set('spark.mongodb.input.uri',
         'mongodb://pdssETLUser:'+urllib.quote('pdssETL@2017')+'@172.28.23.5:27017/fzDB_stg.poc_pyspark_fares_rules?authSource=admin')
...
fare_record = db_fzDB.poc_pyspark_fares_rules
...
system_date = datetime.datetime(2017, 10, 10)  #### Always today's date (SYSTEM DATE) ####
...
ppln_1 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_1 = json.dumps(ppln_1)
cur_record_0 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_1, collection="JUP_DB_ATPCO_Record_0", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
for i in cur_record_0:
    temp = {'carrier': i['CXR_CODE'],'tariff_code': i['TARIFF'], "used":1}
    query.append(temp)

# cur_record_1 = record_1.find()
ppln_2 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_2 = json.dumps(ppln_2)
cur_record_1 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_2, collection="JUP_DB_ATPCO_Record_1", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
ppln_3 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_3 = json.dumps(ppln_3)
cur_record_2_cat_all = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_3, collection="JUP_DB_ATPCO_Record_2_Cat_All", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
ppln_4 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_4 = json.dumps(ppln_4)
cur_record_2_cat_10 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_4, collection="JUP_DB_ATPCO_Record_2_Cat_10", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
#                                                                            "LAST_UPDATED_DATE": query_date_with_dash}],
#                               })
ppln_5 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_5 = json.dumps(ppln_5)
cur_record_2_cat_25 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_5, collection="JUP_DB_ATPCO_Record_2_Cat_25", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
#                                                                            "LAST_UPDATED_DATE": query_date_with_dash}],
#                               })
ppln_6 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_6 = json.dumps(ppln_6)
cur_record_2_cat_3_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_6, collection="JUP_DB_ATPCO_Record_2_Cat_03_FN", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
#                                                                            "LAST_UPDATED_DATE": query_date_with_dash}],
#                               })
ppln_7 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_7 = json.dumps(ppln_7)
cur_record_2_cat_11_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_7, collection="JUP_DB_ATPCO_Record_2_Cat_11_FN", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
#                                                                            "LAST_UPDATED_DATE": query_date_with_dash}],
#                               })
ppln_8 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_8 = json.dumps(ppln_8)
cur_record_2_cat_14_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_8, collection="JUP_DB_ATPCO_Record_2_Cat_14_FN", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
#                                                                            "LAST_UPDATED_DATE": query_date_with_dash}],
#                               })
ppln_9 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_9 = json.dumps(ppln_9)
cur_record_2_cat_15_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_9, collection="JUP_DB_ATPCO_Record_2_Cat_15_FN", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
#                                                                            "LAST_UPDATED_DATE": query_date_with_dash}],
#                               })
ppln_10 = [{"$match": {"DATES_DISC_1":{"$gte":query_date}, "CXR_CODE":{"$in":carrier_list},
                              "$or": [{"DATES_EFF_1":{"$eq":query_date}}, {"DATES_EFF_1": {"$eq": day_before_yesterday},
                                                                           "LAST_UPDATED_DATE": query_date_with_dash}],
                              }}]
ppln_10 = json.dumps(ppln_10)
cur_record_2_cat_23_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_10, collection="JUP_DB_ATPCO_Record_2_Cat_23_FN", database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
for j in range(int(math.ceil(len(query) / 1000.0))):
    ...
    ppln_11 = [{"$match": {"$or": temp}}]
    ppln_11 = json.dumps(ppln_11)
    cur_fare_record = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_11,
                                                                                                   collection="poc_pyspark_fares_rules",
                                                                                                   database="fzDB_stg").load().toPandas().to_dict("records")
    ...
    for i in cur_fare_record:
        ...
        if (i["effective_from"] <= query_date_with_dash and i["effective_to"] == None) or \
           ...
           (i["effective_from"] <= day_before_yesterday_with_dash and i["effective_to"] == day_before_yesterday_with_dash and i["file_date"] == query_date):
            ...
            # cur_record_1 = record_1.find()
            ppln_12 = [{"$match": {"FARE_CLASS": {"$eq": i["fare_basis"]}, "RULE_NO": {"$eq": i["fare_rule"]},
            ...
            ppln_12 = json.dumps(ppln_12)
            cur_record_1 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_12,
                                                                                                    collection="JUP_DB_ATPCO_Record_1",
                                                                                                    database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
...
t = 0
bulk = []
# temp_cur = fare_record.find(, no_cursor_timeout = True)
ppln_13 = [{"$match": {"carrier":{"$in":carrier_list},"used":0}}]
ppln_13 = json.dumps(ppln_13)
temp_cur = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_13,
                                                                                        collection="poc_pyspark_fares_rules",
                                                                                        database="fzDB_stg").load().toPandas().to_dict("records")
...
for i in temp_cur:
    ...
    # cur_record_0 = record_0.find()
    ppln_14 = [{"$match": {"TARIFF": {"$eq": i["tariff_code"]}, "CXR_CODE": {"$eq": i["carrier"]},
                                  "DATES_EFF_1":{"$lte":query_date},
                                  "DATES_DISC_1":{"$gte":query_date}}}]
    ppln_14 = json.dumps(ppln_14)
    cur_record_0 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_14,
                                                                                        collection="JUP_DB_ATPCO_Record_0",
                                                                                        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
    ...
    for h in cur_record_0:
        ...
        for g in range(int(h["NO_SEGS"])):
            # cur_record_2_cat_all = record_2_cat_all.find(
            #     ).sort("SEQ_NO", 1)
            ppln_15 = [{"$match": {"CXR_CODE": {"$eq": i["carrier"]}, "GEN_APPL": {"$eq": ""}, \
            ...
            ppln_15 = json.dumps(ppln_15)
            cur_record_2_cat_all = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(pipeline=ppln_15,
                                                                                                 collection="JUP_DB_ATPCO_Record_2_Cat_All",
                                                                                                 database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            ...
            for j in cur_record_2_cat_all:
                if wild_card_check(i["fare_basis"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "":
                    ...
                    ppln_16 = [{"$match":{'TBL_NO': {'$in': tbl_no_list}}}]
                    ppln_16 = json.dumps(ppln_16)
                    coll_name = "JUP_DB_ATPCO_Record_3_Cat_" + str(j['CAT_NO']).zfill(2)
                    cur_1 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                        pipeline=ppln_16,
    ...
    ppln_17 = [{"$match":{"RULE_NO": {"$eq": i["fare_rule"]}, "CXR_CODE": {"$eq": i["carrier"]}, \
    ...
    ppln_17 = json.dumps(ppln_17)
    cur_record_2_cat_all = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
        pipeline=ppln_17,
        collection="JUP_DB_ATPCO_Record_2_Cat_All",
        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
    cat_check = []
    for j in cur_record_2_cat_all:
        if ((j["CAT_NO"] not in cat_check) and (wild_card_check(i["fare_basis"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "")):
            ...
            # exec "cur_1 = record_3_cat_" + str(j['CAT_NO']) + ".find({'TBL_NO':{'$in':tbl_no_list}})"
            ppln_50= [{"$match": {'TBL_NO': {'$in': tbl_no_list}}}]
            ppln_50 = json.dumps(ppln_50)
            coll_name = "JUP_DB_ATPCO_Record_3_Cat_" + str(j['CAT_NO']).zfill(2)
            cur_1 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_50,
                collection=coll_name,
    ...
    # cur_record_2_cat_10 = record_2_cat_10.find(
    #     ).sort("SEQ_NO", 1)
    ppln_18 = [{"$match": {"RULE_NO": {"$eq": i["fare_rule"]}, "CXR_CODE": {"$eq": i["carrier"]}, \
         "NO_APPL": {"$eq": ""}, "TARIFF": {"$eq": i["tariff_code"]},
         "RTG_NO": {"$in": ["00000", "99999", "88888", i["rtg"]]},
         "OW_RT": {"$in": ["", i["oneway_return"]]},
         "FT_NT": {"$in": ["", i['footnote']]},
    ...
    ppln_18 = json.dumps(ppln_18)
    cur_record_2_cat_all = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
        pipeline=ppln_18,
        collection="JUP_DB_ATPCO_Record_2_Cat_10",
        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
    ...
    for j in cur_record_2_cat_10:
        ...
        if (wild_card_check(i["fare_basis"], j["FARE_CLASS"]) or j["FARE_CLASS"] == ""):
            ...
            # cur_1 = record_3_cat_101.find()
            ppln_19 = [{"$match": {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_19 = json.dumps(ppln_19)
            cur_1 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                        pipeline=ppln_19,
                        collection="JUP_DB_ATPCO_Record_3_Cat_101",
                        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_1:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
            ...
            ppln_20 = [{"$match": {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_20 = json.dumps(ppln_20)
            cur_2 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_20,
                collection="JUP_DB_ATPCO_Record_3_Cat_102",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_2:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
            ...
            ppln_21 = [{"$match": {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_21 = json.dumps(ppln_21)
            cur_3 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_21,
                collection="JUP_DB_ATPCO_Record_3_Cat_103",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_3:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
            ...
            ppln_22 = [{"$match": {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_22 = json.dumps(ppln_22)
            cur_4 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_22,
                collection="JUP_DB_ATPCO_Record_3_Cat_104",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_4:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
            ...
            ppln_23 = [{"$match": {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_23 = json.dumps(ppln_23)
            cur_6 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_23,
                collection="JUP_DB_ATPCO_Record_3_Cat_106",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_6:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
            ...
            ppln_24 = [{"$match": {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_24 = json.dumps(ppln_24)
            cur_7 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_24,
                collection="JUP_DB_ATPCO_Record_3_Cat_107",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_7:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
            ...
            ppln_25 = [{"$match": {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_25 = json.dumps(ppln_25)
            cur_8 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_25,
                collection="JUP_DB_ATPCO_Record_3_Cat_108",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_8:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
            ...
            ppln_26 = [{"$match": {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_26 = json.dumps(ppln_26)
            cur_9 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_26,
                collection="JUP_DB_ATPCO_Record_3_Cat_109",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_9:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
    ...
    #      "LOC_2_COUNTRY": {"$in": [i["origin_country"], i["destination_country"], ""]}
    #      }).sort("SEQ_NO", 1)
    ppln_28 = [{'$match': {"RULE_NO": {"$eq": i["fare_rule"]}, "CXR_CODE": {"$eq": i["carrier"]}, \
         "NO_APPL": {"$eq": ""}, "TARIFF": {"$eq": i["tariff_code"]},
         "RTG_NO":{"$in":["00000","99999","88888",i["rtg"]]},
         "OW_RT":{"$in":["",i["oneway_return"]]},
         "FT_NT":{"$in":["",i['footnote']]},
    ...
    ppln_28 = json.dumps(ppln_28)
    cur_record_2_cat_25 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
        pipeline=ppln_28, collection="JUP_DB_ATPCO_Record_2_Cat_25",
        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
    for j in cur_record_2_cat_25:
        ...
        if wild_card_check(i["fare_basis"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "":
            ...
            ppln_29 = [{'$match': {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_29 = json.dumps(ppln_29)
            cur_1 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_29, collection="JUP_DB_ATPCO_Record_3_Cat_25",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            temp_list = []
            for k in cur_1:
                k["RI"] = ri_list[k['TBL_NO']]
    ...
    #          "LOC_2_COUNTRY": {"$in": [i["origin_country"], i["destination_country"], ""]}
    #          }).sort("SEQ_NO", 1)
    ppln_30 = [{'$match': {"CXR_CODE": {"$eq": i["carrier"]}, "FT_NT": {"$eq": i["footnote"]},
             "TARIFF": {"$eq": i["tariff_code"]}, "NO_APPL": {"$eq": ""},
             "RTG_NO": {"$in": ["00000", "99999", "88888", i["rtg"]]},
             "OW_RT": {"$in": ["", i["oneway_return"]]},
             "DATES_EFF_1":{"$lte":query_date},
    ...
    ppln_30 = json.dumps(ppln_30)
    cur_record_2_cat_3_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
        pipeline=ppln_30, collection="JUP_DB_ATPCO_Record_2_Cat_03_FN",
        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
    for j in cur_record_2_cat_3_fn:
        ...
        if wild_card_check(i["fare_basis"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "":
            ...
            ppln_31 = [{'$match': {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_31 = json.dumps(ppln_31)
            cur_3_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_31, collection="JUP_DB_ATPCO_Record_3_Cat_03_FN",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_3_fn:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
    ...
    #          "LOC_2_COUNTRY": {"$in": [i["origin_country"], i["destination_country"], ""]}
    #          }).sort("SEQ_NO", 1)
    ppln_32 = [{'$match': {"CXR_CODE": {"$eq": i["carrier"]}, "FT_NT": {"$eq": i["footnote"]},
                           "TARIFF": {"$eq": i["tariff_code"]}, "NO_APPL": {"$eq": ""},
                           "RTG_NO": {"$in": ["00000", "99999", "88888", i["rtg"]]},
                           "OW_RT": {"$in": ["", i["oneway_return"]]},
                           "DATES_EFF_1": {"$lte": query_date},
    ...
    ppln_32 = json.dumps(ppln_32)
    cur_record_2_cat_11_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
        pipeline=ppln_32, collection="JUP_DB_ATPCO_Record_2_Cat_11_FN",
        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
    for j in cur_record_2_cat_11_fn:
        ...
        if wild_card_check(i["fare_basis"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "":
            ...
            ppln_33 = [{'$match': {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_33 = json.dumps(ppln_33)
            cur_11_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_33, collection="JUP_DB_ATPCO_Record_3_Cat_11_FN",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_11_fn:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
    ...
    #          "LOC_2_COUNTRY": {"$in": [i["origin_country"], i["destination_country"], ""]}
    #          }).sort("SEQ_NO", 1)
    ppln_34 = [{'$match': {"CXR_CODE": {"$eq": i["carrier"]}, "FT_NT": {"$eq": i["footnote"]},
                           "TARIFF": {"$eq": i["tariff_code"]}, "NO_APPL": {"$eq": ""},
                           "RTG_NO": {"$in": ["00000", "99999", "88888", i["rtg"]]},
                           "OW_RT": {"$in": ["", i["oneway_return"]]},
                           "DATES_EFF_1": {"$lte": query_date},
    ...
    ppln_34 = json.dumps(ppln_34)
    cur_record_2_cat_14_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
        pipeline=ppln_34, collection="JUP_DB_ATPCO_Record_2_Cat_14_FN",
        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
    for j in cur_record_2_cat_14_fn:
        ...
        if wild_card_check(i["fare_basis"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "":
            ...
            ppln_35 = [{'$match': {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_35 = json.dumps(ppln_35)
            cur_14_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_35, collection="JUP_DB_ATPCO_Record_3_Cat_14_FN",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_14_fn:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
    ...
    #          "LOC_2_COUNTRY": {"$in": [i["origin_country"], i["destination_country"], ""]}
    #          }).sort("SEQ_NO", 1)
    ppln_36 = [{'$match': {"CXR_CODE": {"$eq": i["carrier"]}, "FT_NT": {"$eq": i["footnote"]},
                           "TARIFF": {"$eq": i["tariff_code"]}, "NO_APPL": {"$eq": ""},
                           "RTG_NO": {"$in": ["00000", "99999", "88888", i["rtg"]]},
                           "OW_RT": {"$in": ["", i["oneway_return"]]},
                           "DATES_EFF_1": {"$lte": query_date},
    ...
    ppln_36 = json.dumps(ppln_36)
    cur_record_2_cat_15_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
        pipeline=ppln_36, collection="JUP_DB_ATPCO_Record_2_Cat_15_FN",
        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
    for j in cur_record_2_cat_15_fn:
        ...
        if wild_card_check(i["fare_basis"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "":
            ...
            ppln_37 = [{'$match': {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_37 = json.dumps(ppln_37)
            cur_15_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_37, collection="JUP_DB_ATPCO_Record_3_Cat_15_FN",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_15_fn:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
    ...
    #          "LOC_2_COUNTRY": {"$in": [i["origin_country"], i["destination_country"], ""]}
    #          }).sort("SEQ_NO", 1)
    ppln_38 = [{'$match': {"CXR_CODE": {"$eq": i["carrier"]}, "FT_NT": {"$eq": i["footnote"]},
                           "TARIFF": {"$eq": i["tariff_code"]}, "NO_APPL": {"$eq": ""},
                           "RTG_NO": {"$in": ["00000", "99999", "88888", i["rtg"]]},
                           "OW_RT": {"$in": ["", i["oneway_return"]]},
                           "DATES_EFF_1": {"$lte": query_date},
    ...
    ppln_38 = json.dumps(ppln_38)
    cur_record_2_cat_23_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
        pipeline=ppln_38, collection="JUP_DB_ATPCO_Record_2_Cat_23_FN",
        database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
    for j in cur_record_2_cat_23_fn:
        if wild_card_check(i["fare_basis"], j["FARE_CLASS"]) or j["FARE_CLASS"] == "":
            ...
            ppln_39 = [{'$match': {"TBL_NO": {"$in": tbl_no_list}}}]
            ppln_39 = json.dumps(ppln_39)
            cur_23_fn = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_39, collection="JUP_DB_ATPCO_Record_3_Cat_23_FN",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_23_fn:
                k["RI"] = ri_list[k['TBL_NO']]
                k["SEQ_NO"] = j["SEQ_NO"]
    ...
    try:
        for j in range(len(i["cat_4"])):
            ...
            ppln_40 = [{'$match': {"TBL_NO": {"$eq": i["cat_4"][j]["CXR_FLT"]}}}]
            ppln_40 = json.dumps(ppln_40)
            cur_record_3_table_986 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_40, collection="JUP_DB_ATPCO_Record_3_Table_986",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_record_3_table_986:
              i["cat_4"][j]["CXR_FLT"] = k

            # cur_record_3_table_986 = record_3_table_986.find({"TBL_NO": {"$eq": i["cat_4"][j]["CXR_TBL"]}})
            ppln_41 = [{'$match': {"TBL_NO": {"$eq": i["cat_4"][j]["CXR_FLT"]}}}]
            ppln_41 = json.dumps(ppln_41)
            cur_record_3_table_986 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_41, collection="JUP_DB_ATPCO_Record_3_Table_986",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_record_3_table_986:
                i["cat_4"][j]["CXR_TBL"] = k
            countj += 1
        ...
        for j in range(len(i["cat_12"])):
            ...
            ppln_42 = [{'$match': {"TBL_NO": {"$eq": i["cat_12"][j]["CXR_FLT_TBL_NO"]}}}]
            ppln_42 = json.dumps(ppln_42)
            cur_record_3_table_986 = sqlContext.read.format("com.mongodb.spark.sql.DefaultSource").options(
                pipeline=ppln_42, collection="JUP_DB_ATPCO_Record_3_Table_986",
                database="ATPCO_stg_Aug").load().toPandas().to_dict("records")
            for k in cur_record_3_table_986:
