...
File Name: fare_rules_fz.py

This code maps the ATPCO fares to its associated rules and populates the relevant categories in
JUP_DB_ATPCO_Fares_Rules for host whenever full refresh of ATPCO comes.
...
db_ATPCO=pymongo.MongoClient("172.29.104.5:27022")["ATPCO_prod"]
db_ATPCO.authenticate('pdssETLUser', 'pdssETL@123', source='admin')
db_fzDB=pymongo.MongoClient("172.29.104.5:27022")["fzDB_prod"]
...
fares_date = datetime.datetime.strftime(query_date, "%Y-%m-%d")
query_date = datetime.datetime.strftime(query_date, "%Y%m%d")

carrier_list = ['FZ']
st_1 = time.time()
cur_fare_record = fare_record.find({"carrier": {'$in': carrier_list}, 'used': None}, no_cursor_timeout = True)
...
print "stage 1 complete in", time.time() - st_1
...
if len(bulk) > 0:
    ...
    fare_record.bulk_write(bulk)
temp_cur.close()
print "stage 2 complete in ", time.time() - st_2
