...
def get_fares_df(pos, origin, destination, compartment, extreme_start_date, extreme_end_date, db, SYSTEM_DATE, comp=[Host_Airline_Code]):
    ...
    if pos != origin:
        ...
        pos_ = city_airport_df[city_airport_df['Airport_Code'] == origin]['City_Code'].values[0]

        try:
            if currency_data[pos_]['web']:
                currency.append(currency_data[pos_]['web'])
            if currency_data[pos_]['gds']:
                currency.append(currency_data[pos_]['gds'])
        except KeyError:
            currency = []
    else:
        try:
...
if __name__ == '__main__':
    ...
    # for i in trx_date:
    #
    #     update_market('GYD','GYD','DXB','Y', client, i)
    ...
    for idx, row in df_markets.iterrows():
        # if row['pos'] != row['origin']:
        #     city_airport_df = pd.DataFrame(list(db.JUP_DB_City_Airport_Mapping.find({}, {'_id': 0, 'Airport_Code': 1, 'City_Code': 1})))
        #     pos = city_airport_df[city_airport_df['Airport_Code'] == row['origin']]['City_Code'].values[0]
        #     lff.append(run_lff.s(pos=pos, origin=row['origin'], destination=row['od'][3:6], compartment='Y',
        #                          SYSTEM_DATE=row['trx_date']))
        # else:
        #     lff.append(run_lff.s(pos=row['pos'], origin=row['origin'], destination=row['od'][3:6], compartment='Y',
        #                          SYSTEM_DATE=row['trx_date']))
