...
from jupiter_AI.batch.atpco_automation.Automation_tasks import run_fares_and_stage_1, run_record_0_change, \
    ...
    run_stage_2_rules, run_yqyr, run_yqyr_changetab, run_yqyr_tab178190, run_yqyr_tab171198, run_yqyr_secpos, \
    run_yqyr_zoneflag, \
...
from jupiter_AI import client, JUPITER_DB, SYSTEM_DATE, today, parameters, RABBITMQ_HOST, RABBITMQ_PASSWORD, \
    RABBITMQ_PORT, RABBITMQ_USERNAME, ATPCO_DB, JUPITER_LOGGER, ENV, ATPCO_processing_hour, ATPCO_processing_hour_Sunday
import rabbitpy
...
def yqyr(system_date, file_time):
    ...
    for carrier in list(od_list.keys()):
        ...
        for OD in od_list[carrier]:
            ...
            carrier_od.append(
                run_yqyr.s(system_date=system_date, file_time=file_time, carrier=[carrier], OD=[OD], hub=hub,
                           hub_area=hub_area, hub_country=hub_country, hub_zone=hub_zone, origin_area=origin_area,
                           origin_country=origin_country, origin_zone=origin_zone, destination_area=destination_area,
                           destination_country=destination_country, destination_zone=destination_zone))
...
def run_atpco_automation(system_date, file_time):
    ...
    if count_record_4_5 == 0:
        ...
        group_1 = group([run_record_4_5.s(system_date=system_date, file_time=file_time),
                         run_yqyr_changetab.s(system_date=system_date, file_time=file_time)])
        res1 = group_1()
        grp1_res = res1.get()
...
def run_at_file_date_change(system_date, file_time):
    ...
    if count_recseg == 0:
        ...
        db.JUP_DB_Celery_Status.insert({'group_name': 'recseg_group',
                                        ...
                                        'start_time': datetime.datetime.now().strftime('%H:%M'),
                                        'attempt_number': count + 1})
        group_3 = group([run_recseg3.s(system_date=system_date, file_time=file_time),
                         run_recseg2.s(system_date=system_date, file_time=file_time),
                         run_yqyr_zoneflag.s()])
        res3 = group_3()
        grp3_res = res3.get()
    ...
    if count_stage_1_fares == 0:
        ...
        if ENV == 'Staging1':
            ...
            record_0 = db1.JUP_DB_ATPCO_Record_0.aggregate(
            ...
            record_1 = db1.JUP_DB_ATPCO_Record_1.aggregate(
                [{"$match": {"LAST_UPDATED_DATE": system_date, "LAST_UPDATED_TIME": {"$in": file_time}}},
        ...
        with conn.channel() as channel:
            ...
            for message in queue:
                ...
                if received_msg['file_date'] == YESTERDAY or received_msg['file_date'] == TODAY_:
                    ...
                    SYSTEM_DATE_1 = received_msg['file_date']
                    fr_date = datetime.datetime.strptime(SYSTEM_DATE_1, '%Y-%m-%d')
                    if "file_time" in received_msg:
                        file_time = received_msg['file_time']
                    elif (fr_date.weekday() == 6 or fr_date.weekday() == 5):
                        file_time = "17"
                    else:
                        file_time = "20"
                    ...
                    if (time_in_range(datetime.time(int(ATPCO_processing_hour['run_0']['start']), 0, 0),
                                      datetime.time(int(ATPCO_processing_hour['run_0']['end']), 0, 0),
                                      datetime.time(int(file_time), 0, 0))):
                        ...
                        # message.ack()
                    elif (time_in_range(datetime.time(int(ATPCO_processing_hour['run_1']['start']), 0, 0),
                                        datetime.time(int(ATPCO_processing_hour['run_1']['end']), 0, 0),
                                        datetime.time(int(file_time), 0, 0))):
                        hours.append(int(file_time))
                        hours_str.append(file_time)
                        run_atpco_automation(SYSTEM_DATE_1, file_time)
                        list_hour.append(file_time)
                        # message.ack()
                    elif (time_in_range(datetime.time(int(ATPCO_processing_hour['run_2']['start']), 0, 0),
                                        datetime.time(int(ATPCO_processing_hour['run_2']['end']), 0, 0),
                                        datetime.time(int(file_time), 0, 0))):
                        hours.append(int(file_time))
                        run_atpco_automation(SYSTEM_DATE_1, file_time)
                        list_hour.append(file_time)
                    ...
                    eachdep = datetime.datetime.strptime(SYSTEM_DATE_1, '%Y-%m-%d')
                    # Sunday we don't get files all hour files since we have to change batch-3 end file time
                    if (int(eachdep.isoweekday()) == 6):
                        if (int(file_time) == int(ATPCO_processing_hour_Sunday['run_2']['end'])):
                            ...
                            if len(mail_array) > 0:
                                ...
                                rules_volume = atpco.JUP_DB_ATPCO_Record_1.find(
                                    {'LAST_UPDATED_DATE': SYSTEM_DATE_1,
                                     "LAST_UPDATED_TIME": {'$in': hours_str}}).count()
                                rules_volume += atpco.JUP_DB_ATPCO_Record_0.find(
                                    {'LAST_UPDATED_DATE': SYSTEM_DATE_1,
                                     "LAST_UPDATED_TIME": {'$in': hours_str}}).count()
                                rules_volume += atpco.JUP_DB_ATPCO_Record_2_Cat_All.find(
