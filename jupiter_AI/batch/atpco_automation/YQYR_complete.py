...
def stage1_hub1(cxr, origin, origin_area, origin_zone, origin_country, destination, destination_area,
                destination_zone, destination_country, system_date, hub, hub_area, hub_zone, hub_country, client):
    print "============HUB1 stage1============="
    db_ATPCO = client[ATPCO_DB]
    yqyr = db_ATPCO.JUP_DB_ATPCO_YQYR_RecS1
...
def yqyr_hub(system_date, file_date, filedate, carrier_list, OD_list, hub, hub_area, hub_country, hub_zone, origin,
...
# @measure(JUPITER_LOGGER)
def yqyr_p2p(system_date, file_date, filedate, carrier_list, OD_list, origin, origin_area, origin_country, origin_zone,
    ...
    df_stage1 = stage1_p2p(carrier_list[0], origin, origin_area, origin_zone, origin_country, destination,
                           destination_area,
                           destination_zone, destination_country, system_date, client)
    if df_stage1.empty:
...
def yqyr_complete_main(system_date, carrier_list, OD_list, hub, hub_area, hub_country, hub_zone, origin_area, origin_country,
    ...
    if hub in [origin, destination]:
        yqyr_p2p(system_date, file_date, filedate, carrier_list, OD_list, origin, origin_area, origin_country,
                 origin_zone, destination, destination_area, destination_country, destination_zone, client)

    else:
        yqyr_hub(system_date, file_date, filedate, carrier_list, OD_list, hub, hub_area, hub_country, hub_zone, origin,
                 origin_area, origin_country, origin_zone, destination, destination_area, destination_country,
                 destination_zone, client)
