...
def stage1_hub1(cxr, origin, origin_area, origin_zone, origin_country, destination, destination_area,
                destination_zone, destination_country, system_date, file_time, hub, hub_area, hub_zone, hub_country,
                client):
    print "============HUB1 stage1============="
    db_ATPCO = client[ATPCO_DB]
    yqyr = db_ATPCO.JUP_DB_ATPCO_YQYR_RecS1
...
def stage1_p2p(cxr, origin, origin_area, origin_zone, origin_country, destination, destination_area, destination_zone,
               destination_country, system_date, file_time, client):
    print "============P2P stage 1============="
    db_ATPCO = client[ATPCO_DB]
    yqyr = db_ATPCO.JUP_DB_ATPCO_YQYR_RecS1
    cur2 = yqyr.find({"$and": [{"flag": 1}, {"CXR_CODE": cxr}, {"DISC_DATE": {"$gte": system_date}},
                                        ...
                                        {'$and': [
                                            ...
                                            {"$or": [{"JRNY_LOC_2_ZONE_TABLE_178.0.GEO_LOC_CITY": {"$eq": destination}},
                                                     ...
                                                     {"JRNY_LOC_2_ZONE_TABLE_178.0.GEO_LOC_COUNTRY": {
                                                         "$eq": destination_country}},
                                                     {'$and': [
                                                         {"JRNY_LOC_2_ZONE_TABLE_178.0.GEO_LOC_AREA": {'$size': 0}},
                                                         {"JRNY_LOC_2_ZONE_TABLE_178.0.GEO_LOC_CITY": {'$size': 0}},
...
def yqyr_hub(system_date, file_time, file_date, filedate, carrier_list, OD_list, hub, hub_area, hub_country, hub_zone,
             origin,
             origin_area, origin_country, origin_zone, destination, destination_area, destination_country,
    ...
    df_stage1_hub1 = stage1_hub1(carrier_list[0], origin, origin_area, origin_zone, origin_country, destination,
                                 destination_area, destination_zone, destination_country, system_date, file_time, hub,
                                 hub_area,
                                 hub_zone, hub_country, client)
    ...
    cur = coll.find(
        {'carrier': {'$in': carrier_list}, 'file_date': filedate, "file_time": {"$in" : file_time}, 'OD': {'$in': OD_list},
         "$where": """function() {       
        return this.effective_to >= this.effective_from || this.effective_to == null
        }"""
         },{"oneway_return"  : 1,'RBD' : 1,'fare' : 1,'fare_basis' : 1,'currency' : 1,'compartment' : 1,'Reference_Rate' : 1,"carrier" : 1,"origin" : 1,"origin_area" : 1,"origin_zone" : 1,"origin_country" : 1,"destination" : 1,"destination_area" : 1,"destination_zone" : 1,"destination_country" : 1,"_id" : 1},
...
# @measure(JUPITER_LOGGER)
def yqyr_p2p(system_date, file_time, file_date, filedate, carrier_list, OD_list, origin, origin_area, origin_country,
             origin_zone,
             destination, destination_area, destination_country, destination_zone, client):
    print "============P2P============="
    db_ATPCO = client[ATPCO_DB]
    db_fzDB = client[JUPITER_DB]
    ...
    df_stage1 = stage1_p2p(carrier_list[0], origin, origin_area, origin_zone, origin_country, destination,
                           destination_area,
                           destination_zone, destination_country, system_date, file_time, client)
    if df_stage1.empty:
...
# @measure(JUPITER_LOGGER)
def yqyr_main(system_date,file_time, carrier_list, OD_list, hub, hub_area, hub_country, hub_zone, origin_area, origin_country,
    ...
    filedate = datetime.datetime.strftime(sysdate, "%Y%m%d")
    ...
    if hub in [origin, destination]:
        yqyr_p2p(system_date, file_time, file_date, filedate, carrier_list, OD_list, origin, origin_area,
                 origin_country,
                 origin_zone, destination, destination_area, destination_country, destination_zone, client)

    else:
        yqyr_hub(system_date, file_time, file_date, filedate, carrier_list, OD_list, hub, hub_area, hub_country,
                 hub_zone, origin,
                 origin_area, origin_country, origin_zone, destination, destination_area, destination_country,
                 destination_zone, client)
...
if __name__ == '__main__':
    ...
    for carrier in list(od_list.keys()):
        ...
        for OD in od_list[carrier]:
            ...
            yqyr_main(system_date,file_time, [carrier], [OD], hub, hub_area, hub_country, hub_zone, origin_area,
