...
def run(db, markets, sig_flag=None):
    ...
    st_trend = time.time()
    trend_triggers = generate_trend_triggers(markets=markets, sig_flag=sig_flag, db=db)
    st_opp = time.time()
    opp_triggers = []
    opp_triggers = raise_opportunities_trigger(markets=markets, sig_flag=sig_flag, db=db)
    # print "Time taken to generate PCCP TRIGGERS = ", time.time() - st_pccp
    print "Time taken to generate TREND TRIGGERS = ", st_opp - st_trend
    print "Time taken to generate Opportunites TRIGGERS = ", time.time() - st_opp

    TRIGGERS_ALL = trend_triggers + opp_triggers
    df = pd.DataFrame()
...
if __name__ == '__main__':
    ...
    markets = [ "KWIKWIWASY","KWIKWIYTOY",'KWIKWINYCY','KWIKWIPARY','KWIKWISAOY']

    run(db=db, markets=markets, sig_flag='sig')
    import sys
    client.close()
    sys.exit()
