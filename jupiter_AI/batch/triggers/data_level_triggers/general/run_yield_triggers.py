...
@measure(JUPITER_LOGGER)
def generate_yield_triggers(pos, origin, destination, compartment, db, data_list, sig_flag=None):
    global TRIGGERS
    try:
        print "YWK"
        ywk_triggers = generate_yield_changes_weekly_triggers(pos=pos,
                                                              origin=origin,
    ...
    try:
        print "YRT"
        yrt_triggers = generate_yield_changes_rolling_triggers(pos=pos,
                                                               origin=origin,
                                                               destination=destination,
                                                               compartment=compartment,
                                                               data_list=data_list,
                                                               db=db,
                                                               sig_flag=sig_flag)
        TRIGGERS = TRIGGERS + yrt_triggers
    except Exception as error_msg:
        module_name = ''.join(
            [
                'jupiter_AI/batch/triggers/data_level_triggers/general/daily.py ',
                'method: run:yield_changes_rolling::',
                pos,
                '/',
                origin,
                '/',
    ...
    try:
        print "YVLYR"
        yvlyr_triggers = generate_yield_changes_vlyr_triggers(pos=pos,
                                                              origin=origin,
                                                              destination=destination,
                                                              compartment=compartment,
                                                              data_list=data_list,
                                                              db=db,
                                                              sig_flag=sig_flag)
        TRIGGERS = TRIGGERS + yvlyr_triggers
    except Exception as error_msg:
        module_name = ''.join(
            [
                'jupiter_AI/batch/triggers/data_level_triggers/general/daily.py ',
    ...
    try:
        print "YVTGT"
        yvtgt_triggers = generate_yield_changes_vtgt_triggers(pos=pos,
                                                              origin=origin,
                                                              destination=destination,
                                                              compartment=compartment,
                                                              data_list=data_list,
                                                              db=db,
                                                              sig_flag=sig_flag)
        TRIGGERS = TRIGGERS + yvtgt_triggers
    except Exception as error_msg:
        module_name = ''.join(
            [
                'jupiter_AI/batch/triggers/data_level_triggers/general/daily.py ',
                'method: run:yield_changes_VTGT::',
                pos,
                '/',
                origin,
                '/',
...
def run_data_level_triggers(pos, origin, destination, compartment, db, data_list, sig_flag=None):
    ...
    st_yield = time.time()
    print "Generating Yield Triggers . . . . . . ."
    generate_yield_triggers(pos=pos,
                            origin=origin,
                            destination=destination,
                            compartment=compartment,
                            data_list=data_list,
                            db=db,
                            sig_flag=sig_flag)
    print "Time taken to generate Yield Triggers = ", time.time() - st_yield
