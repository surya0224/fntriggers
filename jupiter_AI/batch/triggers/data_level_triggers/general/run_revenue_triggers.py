...
def generate_revenue_triggers(
    ...
    # try:
    #     print "RVWK"
    #     rvwk_triggers = generate_revenue_changes_weekly_triggers(pos=pos,
    #                                                              origin=origin,
    ...
    try:
        print "RVRT"
        rvrt_triggers = generate_revenue_changes_rolling_triggers(pos=pos,
                                                                  origin=origin,
                                                                  destination=destination,
                                                                  compartment=compartment,
                                                                  data_list=data_list,
                                                                  db=db,
                                                                  sig_flag=sig_flag)
        TRIGGERS = TRIGGERS + rvrt_triggers
    except Exception as error_msg:

        module_name = ''.join(
            [
                'jupiter_AI/batch/triggers/data_level_triggers/general/daily.py ',
                'method: run:revenue_changes_rolling::',
                pos,
                '/',
                origin,
                '/',
    ...
    try:
        print "RVVLYR"
        rvvlyr_triggers = generate_revenue_changes_vlyr_triggers(pos=pos,
                                                                 origin=origin,
                                                                 destination=destination,
                                                                 compartment=compartment,
                                                                 db=db,
                                                                 data_list=data_list,
                                                                 sig_flag=sig_flag)
        TRIGGERS = TRIGGERS + rvvlyr_triggers
    except Exception as error_msg:

        module_name = ''.join(
            [
                'jupiter_AI/batch/triggers/data_level_triggers/general/daily.py ',
                'method: run:revenue_changes_VLYR::',
                pos,
                '/',
                origin,
                '/',
    ...
    try:
        print "RVVTGT"
        rvvtgt_triggers = generate_revenue_changes_vtgt_triggers(pos=pos,
                                                                 origin=origin,
                                                                 destination=destination,
                                                                 compartment=compartment,
                                                                 data_list=data_list,
                                                                 db=db,
                                                                 sig_flag=sig_flag)
        TRIGGERS = TRIGGERS + rvvtgt_triggers
    except Exception as error_msg:
        module_name = ''.join(
            [
                'jupiter_AI/batch/triggers/data_level_triggers/general/daily.py ',
                'method: run:revenue_changes_VTGT::',
                pos,
                '/',
                origin,
                '/',
...
def run_data_level_triggers(pos, origin, destination, compartment, db, data_list, sig_flag=None):
    ...
    st_revenue = time.time()
    print "Generating Revenue Triggers . . . . . "
    generate_revenue_triggers(pos=pos,
                              origin=origin,
                              destination=destination,
                              compartment=compartment,
                              db=db,
    ...
    print "Time taken to generate Revenue Triggers = ", time.time() - st_revenue
...
def run(db, markets, sig_flag=None):
    ...
    # #
    # print 'No of pos/od/compartment combinations', len(market)
...
# , db=db, sig_flag='sub_sig')
  ...
  "DELDELBOGJ",
        ...
        # 'KWIKWISYDY', 'KWIKWITHRY', 'KWIKWILONY', 'KWIKWIKULY', 'KWIKWIJNBY', 'KWIKWIDXBY'
