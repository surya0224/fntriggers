...
File Name: copper_flights.py

This code analyzes the copper flights that have been running with near full capacity for a significant period of
time and populates the JUP_DB_Copper_Flights collection daily as a batch file.
...
def get_threshold_values(db):
    ...
    crsr = db.JUP_DB_Thresholds.find({"item": "Copper Flights", "active": True,
...
@measure(JUPITER_LOGGER)
def get_copper_flights_query(valid_start_date, valid_end_date, seat_factor_upper_threshold,
...
def add_user_defined_flights(user_defined_flights, db):
    for j in user_defined_flights:
        ...
        if len(inv_cur) > 0:
            ...
            db.JUP_DB_Copper_Flights.insert(dict_to_insert)
        else:
            print 'No record found in Inventory leg'


@measure(JUPITER_LOGGER)
def generate_copper_flights(client):
    ...
    result = db.JUP_DB_Copper_Flights.delete_many({})
    print "removed", result.deleted_count
    query_pipeline_y, query_pipeline_j = get_copper_flights_query(valid_start_date, valid_end_date,
    ...
    for i in cursor:
        if i['high_sf_percentage'] >= frequency:
            ...
            db.JUP_DB_Copper_Flights.insert(i)
