...
import datetime
from jupiter_AI import JUPITER_DB, mongo_client, Host_Airline_Code, JUPITER_LOGGER
...
@measure(JUPITER_LOGGER)
def get_fares_df(pos, origin, destination, compartment, extreme_start_date, extreme_end_date, db, SYSTEM_DATE, comp=[Host_Airline_Code]):
    currency_data = get_currency_data(db)
    currency = []
    try:
...
def get_lowest_filed_fare_dF(
    ...
    for date_ in date_list:
        ...
        min_df = lowest_fares_d.loc[
            lowest_fares_d.groupby(by=['oneway_return', 'currency', 'carrier', 'channel'])['Average_fare'].idxmin()]
        max_df = lowest_fares_d.loc[
            lowest_fares_d.groupby(by=['oneway_return', 'currency', 'carrier', 'channel'])['Average_fare'].idxmax()]
        ...
        min_col_list = ['carrier', 'currency', 'channel','lowest_fare_total', 'lowest_fare_YQ',
                        'lowest_fare_YR', 'lowest_fare_surcharge', 'lowest_fare_base', 'lowest_fare_fb']
        max_col_list = ['carrier', 'currency', 'channel', 'highest_fare_total', 'highest_fare_YQ',
        ...
        min_df = pd.merge(min_df, min_df_rt, on=['carrier', 'currency', 'channel'], how='outer', suffixes=("", "_rt"))
        max_df = pd.merge(max_df, max_df_rt, on=['carrier', 'currency', 'channel'], how='outer', suffixes=("", "_rt"))

        temp_df = pd.merge(min_df, max_df, on=['carrier', 'currency', 'channel'], how='outer')
        ...
        # count += 1
    if len(final_data) > 0:
        final_data['lowest_fare_total'] = final_data.apply(lambda row: row['lowest_fare_total'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['lowest_fare_YQ'] = final_data.apply(lambda row: row['lowest_fare_YQ'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['lowest_fare_YR'] = final_data.apply(lambda row: row['lowest_fare_YR'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['lowest_fare_surcharge'] = final_data.apply(lambda row: row['lowest_fare_surcharge'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['lowest_fare_base'] = final_data.apply(lambda row: row['lowest_fare_base'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)


        final_data['lowest_fare_total_rt'] = final_data.apply(lambda row: row['lowest_fare_total_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['lowest_fare_YQ_rt'] = final_data.apply(lambda row: row['lowest_fare_YQ_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['lowest_fare_YR_rt'] = final_data.apply(lambda row: row['lowest_fare_YR_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['lowest_fare_surcharge_rt'] = final_data.apply(lambda row: row['lowest_fare_surcharge_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['lowest_fare_base_rt'] = final_data.apply(lambda row: row['lowest_fare_base_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)

        final_data['highest_fare_total'] = final_data.apply(lambda row: row['highest_fare_total'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['highest_fare_YQ'] = final_data.apply(lambda row: row['highest_fare_YQ'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['highest_fare_YR'] = final_data.apply(lambda row: row['highest_fare_YR'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['highest_fare_surcharge'] = final_data.apply(lambda row: row['highest_fare_surcharge'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['highest_fare_base'] = final_data.apply(lambda row: row['highest_fare_base'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)

        final_data['highest_fare_total_rt'] = final_data.apply(lambda row: row['highest_fare_total_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['highest_fare_YQ_rt'] = final_data.apply(lambda row: row['highest_fare_YQ_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['highest_fare_YR_rt'] = final_data.apply(lambda row: row['highest_fare_YR_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['highest_fare_surcharge_rt'] = final_data.apply(lambda row: row['highest_fare_surcharge_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)
        final_data['highest_fare_base_rt'] = final_data.apply(lambda row: row['highest_fare_base_rt'] * (
                final_data['currency'].map(exchange_rate) / exchange_rate[currency_data[pos]['web']]), axis=1)

        final_data['currency'] = currency_data[pos]['web']
        #print final_data.to_string()
        ...
        filed_df = final_data[final_data['channel'] == "web"]
        gds_df = final_data[final_data['channel'] == "gds"]
        if len(filed_df)>0 and len(gds_df)>0:
            gds_df.rename(columns={"currency":"currency_gds",
                    "highest_fare_YQ":"highest_fare_YQ_gds",
                    "highest_fare_YR":"highest_fare_YR_gds",
                    "highest_fare_base":"highest_fare_base_gds",
                    "highest_fare_fb":"highest_fare_fb_gds",
                    "highest_fare_surcharge":"highest_fare_surcharge_gds",
                    "highest_fare_total":"highest_fare_total_gds",
                    "lowest_fare_YQ":"lowest_fare_YQ_gds",
                    "lowest_fare_YR":"lowest_fare_YR_gds",
                    "lowest_fare_base":"lowest_fare_base_gds",
                    "lowest_fare_fb":"lowest_fare_fb_gds",
                    "lowest_fare_surcharge":"lowest_fare_surcharge_gds",
                    "lowest_fare_total":"lowest_fare_total_gds",
                    "highest_fare_YQ_rt":"highest_fare_YQ_rt_gds",
                    "highest_fare_YR_rt":"highest_fare_YR_rt_gds",
                    "highest_fare_base_rt":"highest_fare_base_rt_gds",
                    "highest_fare_fb_rt":"highest_fare_fb_rt_gds",
                    "highest_fare_surcharge_rt":"highest_fare_surcharge_rt_gds",
                    "highest_fare_total_rt":"highest_fare_total_rt_gds",
                    "lowest_fare_YQ_rt":"lowest_fare_YQ_rt_gds",
                    "lowest_fare_YR_rt":"lowest_fare_YR_rt_gds",
                    "lowest_fare_base_rt":"lowest_fare_base_rt_gds",
                    "lowest_fare_fb_rt":"lowest_fare_fb_rt_gds",
                    "lowest_fare_surcharge_rt":"lowest_fare_surcharge_rt_gds",
                    "lowest_fare_total_rt":"lowest_fare_total_rt_gds",
                    }, inplace=True)
            gds_df.drop(['channel'], axis=1, inplace=True)
            filed_df.drop(['channel'], axis=1, inplace=True)
            final_data= pd.merge(filed_df, gds_df, how="outer", on=['carrier', 'date_start', 'date_end'])
            ...
            filed_df = pd.DataFrame(columns=[u'carrier', u'currency', u'lowest_fare_total', u'lowest_fare_YQ',
                                       u'lowest_fare_YR', u'lowest_fare_surcharge', u'lowest_fare_base',
                                       u'lowest_fare_fb', u'lowest_fare_total_rt', u'lowest_fare_YQ_rt',
                                       u'lowest_fare_YR_rt', u'lowest_fare_surcharge_rt',
                                       u'lowest_fare_base_rt', u'lowest_fare_fb_rt', u'highest_fare_total',
                                       ...
                                       u'highest_fare_YQ_rt', u'highest_fare_YR_rt',
                                       u'highest_fare_surcharge_rt', u'highest_fare_base_rt',
                                       u'highest_fare_fb_rt', u'date_start', u'date_end',])
            final_data = pd.merge(filed_df, gds_df, how="outer", on=['carrier', 'date_start', 'date_end'])
...
@measure(JUPITER_LOGGER)
def update_market(pos, origin, destination, compartment, client, SYSTEM_DATE):
    ...
    mt_cur = list(db.JUP_DB_Manual_Triggers_Module.aggregate([
        {
            '$match': {'pos.City': pos, 'origin.City': origin, 'destination.City': destination,
                       'compartment.compartment': compartment, 'trx_date': SYSTEM_DATE}
        },
        {
            '$group': {
                '_id': None,
...
if __name__ == '__main__':
    ...
    # lff = []
    from jupiter_AI.batch.atpco_automation.Automation_tasks import run_lff_snap_date
    ...
    # lff = []
    # for i in trx_date:
    #     lff.append(run_lff.s(pos="GYD", origin="GYD", destination="DXB", compartment="Y", SYSTEM_DATE=i))
    # group2 = group(lff)
    # res2 = group2()
    client = mongo_client()
    # update_market('CMB','CMB','DXB','Y', client, SYSTEM_DATE="2017-09-22")

    db = client[JUPITER_DB]

    df_markets = pd.DataFrame(list(db.Temp_fzDB_tbl_005.find({})))
    lff = []
    for idx, row in df_markets.iterrows():
        lff.append(run_lff_snap_date.s(pos=row['pos'], origin=row['origin'],
                             destination=row['destination'],
                             compartment=row['compartment'],
                             SYSTEM_DATE=row['trx_date']))
    group2 = group(lff)
    res2 = group2()
