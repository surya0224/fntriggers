...
def run():
    ...
    while count_batch < total_markets:
        ...
        cur1 = pd.DataFrame(list(db.JUP_DB_OD_Capacity.find({"pseudo_od":{'$in':ods},"month_year": SYSTEM_DATE[0:4]+SYSTEM_DATE[5:7],"compartment":{'$in':['Y','J']}}
                    ,{'od':1,'od_block_time':1,'pseudo_od':1,'compartment':1,'carrier':1,'_id':0 })))
        ...
        if len(cur1) != 0:
            ...
            for odc, values in grouped_df:
                val = values['od_block_time'].values
