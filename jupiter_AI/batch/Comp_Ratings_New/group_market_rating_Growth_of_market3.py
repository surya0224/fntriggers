...
def run():
    ...
    def main():
            ...
            for compartment in dict_mkt_share_recs:
                ...
                for pseudo_od in dict_mkt_share_recs[compartment]:
                        for pos in dict_mkt_share_recs[compartment][pseudo_od]:
                            ...
                            total_growths=growth_of_market(market_share_data, 2017) #[rec[1], rec[2], rec[3], len(dict_airlines), dict_airlines, rec[6]], rec1=comp,rec2=pseudo_od,rec3=pos, rec[6]=pseudo_od2
                            if total_growths is not None:
                                list_growths.append(total_growths)
        ...
        #print 'p81', list_growths
        list_ratings=rank_competitors_across_markets(list_growths, dict_compartments)


        update_Data_Competitor_Ratings_collection(list_growths, pseudo_ods_map)
    ...
    def read_mkt_share_data(list_ods, pseudo_ods_map):
        ...
        for c in cursor:
            ...
            for i in c['top_5_comp_data']:
                ...
                dict_mkt_share_recs[compartment][pseudo_od][pos][airline][year][month]['pax'] = float(pax)
        ...
        for airline in dict_mkt_share_recs[compartment][pseudo_od][pos]:
            for year in dict_mkt_share_recs[compartment][pseudo_od][pos][airline]:
                for month in dict_mkt_share_recs[compartment][pseudo_od][pos][airline][year]:
                    pax=dict_mkt_share_recs[compartment][pseudo_od][pos][airline][year][month]['pax']
                    rec=[]
                    rec.append(airline)
    ...
    @measure(JUPITER_LOGGER)
    def growth_of_market(mkt_recs, ayear):
        prvyear=ayear-1
        dict_airlines={}
        ...
        for rec in mkt_recs:
            if rec[4] != ayear: #rec[4] is year
                continue
            total_pax+=rec[7]  #total pax across all airlines for year=ayear

        prv_pax=0.0
        for rec in mkt_recs:
            if rec[4] != prvyear: #rec[4] is year
                continue
            dict_airlines[rec[0]]=1
            prv_pax+=rec[7]  #total pax across all airlines for year=prvyear

        if prv_pax==0:
            growth=0.0
        else:
            growth=total_pax/prv_pax-1.0

        if len(dict_airlines)==0:
            return None
        market_growth=[rec[1], rec[2], rec[3], growth, dict_airlines, rec[6]]
    ...
    def rank_competitors_across_markets(list_recs, dict_compartments):
        ...
        for compartment in dict_compartments:
            ...
            for rec in compart_recs:
                #print 'p80', rec
                list_ratings.append(rec[3]) #total_pax
                maxi=max(maxi, rec[3])
            print 'x19', maxi
    ...
    @measure(JUPITER_LOGGER)
    def update_Data_Competitor_Ratings_collection(list_growths, pseudo_ods_map):
        for icnt in range(len(list_growths)):
            rec=list_growths[icnt]
            pseudo_od = rec[1]
            for od in pseudo_ods_map[pseudo_od]:
                ...
                dict_mkt_share_rating_rec['component'] = 'Growth of Market'
                dict_mkt_share_rating_rec['total_growths']=rec[3]
                dict_airlines=rec[4]
                float_ratings=rec[6]
