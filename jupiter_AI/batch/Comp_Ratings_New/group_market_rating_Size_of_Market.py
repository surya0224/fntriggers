...
def run():
    ...
    def main():
            ...
            for compartment in dict_mkt_share_recs:
                ...
                for pseudo_od in dict_mkt_share_recs[compartment]:
                        for pos in dict_mkt_share_recs[compartment][pseudo_od]:
                            ...
                            total_paxes=size_of_market(market_share_data, 2017) #[rec[1], rec[2], rec[3], len(dict_airlines), dict_airlines, rec[6]], rec1=comp,rec2=pseudo_od,rec3=pos, rec[6]=pseudo_od2
                            if total_paxes is not None:
                                list_paxes.append(total_paxes)
        ...
        #print 'p81', list_paxes
        list_ratings=rank_competitors_across_markets(list_paxes, dict_compartments)



        update_Data_Competitor_Ratings_collection(list_paxes, pseudo_ods_map)
    ...
    @measure(JUPITER_LOGGER)
    def size_of_market(mkt_recs, ayear):
        dict_airlines={}
        total_pax=0.0
        for rec in mkt_recs:
        ...
        if len(dict_airlines)==0:
            return None
        market_pax=[rec[1], rec[2], rec[3], total_pax, dict_airlines, rec[6]]
    ...
    @measure(JUPITER_LOGGER)
    def update_Data_Competitor_Ratings_collection(list_paxes, pseudo_ods_map):
        for icnt in range(len(list_paxes)):
            rec=list_paxes[icnt]
            pseudo_od = rec[1]
            for od in pseudo_ods_map[pseudo_od]:
                ...
                dict_mkt_share_rating_rec['component'] = 'Size of Market'
                dict_mkt_share_rating_rec['total_paxes']=rec[3]
                dict_airlines=rec[4]
                float_ratings=rec[6]
