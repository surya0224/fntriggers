...
import datetime
import json
from jupiter_AI import Host_Airline_Hub,Host_Airline_Code
import time
...
def main(dict_mkts_to_be_printed):
    ...
    for i in range(10000):  # 10000 is number of iterations, so max number of ods computed = 10000*nods_in_batch
        ...
        print 'L5a processing batch # ', i, 'starting from', starting_number, 'to', ending_number, 'starting od', \
        chosen_ods[0], 'ending origin', chosen_ods[len(chosen_ods) - 1]
        # main loop that calculates the rating for each market within a given batch of od's
        tot_mkts = batch_loop(chosen_ods, df_nonod_ratings_global, df_grp_wts, df_comp_wts, tot_mkts,
                              dict_mkts_to_be_printed)
...
def prepare_global_wts():
    ...
    df_comp_wts.set_index(['ix_compartment', 'ix_region', 'ix_country', 'ix_od', 'ix_group', 'ix_component'],
                          inplace=True)
...
def correct_component_names(df):
    ...
    for ix in df.index:
        ...
        if df.loc[ix, 'group'] == 'Distributors Rating' and df.loc[ix, 'region'] == None and df.loc[
            ix, 'country'] == None and df.loc[ix, 'pos'] == None:
            df.loc[ix, 'rating_definition_level'] = 'pos_od_compartment'

        if df.loc[ix, 'group'] == 'Fares Rating' and df.loc[ix, 'region'] == None and df.loc[ix, 'country'] == None and \
                        df.loc[ix, 'pos'] == None:
            df.loc[ix, 'rating_definition_level'] = 'pos_od_compartment'
...
def normalize_grp_wts(df):
    ...
    df2 = ser.to_frame()
    df2.reset_index(
        inplace=True)  # converts index into columns; inplace=True does the replacement on the dataframe df itself (without it a copy of dataframe would be returned)
...
def batch_loop(chosen_ods, df_nonod_ratings_global, df_grp_wts, df_comp_wts, tot_mkts, dict_mkts_to_be_printed):
    ...
    for ix in df.index:
        if df.loc[ix, 'pos'] is not None:
            dict_mkts[
                (df.loc[ix, 'compartment'], df.loc[ix, 'origin'], df.loc[ix, 'destination'], df.loc[ix, 'pos'])] = 1
    ...
    for mkt in dict_mkts:
        ...
        df_template, od_airlines = build_template(mkt, df_od_ratings_batch, df_nonod_ratings_global, df_grp_wts,
                                                  df_comp_wts)
...
def get_ratings_from_collection(query):
    ...
    if query == None:
        cursor = db.JUP_DB_Data_Competitor_Ratings1.find({'origin': None, 'destination': None})
        ...
        cursor = db.JUP_DB_Data_Competitor_Ratings1.find(query)
    ...
    df = pd.DataFrame(list(cursor))
    df = df[(df['compartment'] != 'NA') & (df['compartment'] != 'N/A') & (df['compartment'] != 'na') & (
    df['compartment'] != 'n/a') & (df['compartment'] != 'Others')]
    if query == None:
        df.loc[:, 'pos'] = None
...
def build_template(mkt, df_od_ratings_batch, df_nonod_ratings_global, df_grp_wts, df_comp_wts):
    ...
    df_template = seed_template(compartment, od,
                                df_comp_wts)  # retrieves component weights for this market from df_comp_wts (which is for entire batch of markets)
    ...
    df_template = pd.merge(df_template, df_grp_wts[['compartment', 'group', 'norm_grp_wt']],
                           on=['compartment', 'group'], how='left')
    ...
    df_template = pd.merge(df_template, df_tmp[['group', 'component', 'rating_definition_level']],
                           on=['group', 'component'], how='left')
    ...
    df_template = pd.merge(df_template, df_ratings_tmp[['group', 'component', 'ratings']], on=['group', 'component'],
                           how='left')
    # print 'M36a template with od ratings (without imputing) and weights', "(" + str(df_template.compartment.count())+" recs)", df_template
    ...
    df_template = pd.merge(df_template, df_nonod_ratings_global[['compartment', 'group', 'component', 'ratings']],
                           on=['compartment', 'group', 'component'], how='left')
...
def calculate_ratings(df_template, od_airlines, mkt):
    airline_ratings = {}
    for airline in od_airlines:
        airline_ratings[airline] = (
        df_template[airline] * df_template['norm_grp_wt'] * df_template['norm_comp_wt']).sum()
...
def update_collection(mkt, df_grp_wts, airline_ratings, group_ratings):
    ...
    print "--------------------", len(dict_rec)
    print dict_rec[:2]
    db.JUP_DB_Competitor_Ratings1.insert(dict_rec)
